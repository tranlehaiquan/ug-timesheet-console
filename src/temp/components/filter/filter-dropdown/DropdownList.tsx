...
import { ListTypes, Options, DropdownListProps, Omit, ArrayOptionRendererProps } from '../interfaces'
...
const InputOptionRenderer: React.FC<Omit<DropdownListProps, 'options'> & { options: Options }> = ({
  listType,
  onSearch,
  onApply,
  ...
  const [allowApply, setAllowApply] = React.useState(values(options).includes(true))
  const [visibleApply, setVisibleApply] = React.useState(visibleApplyButton)

  const searchHandler = onSearch || (({ target }) => setSearchValue(target.value))

  const hideButton = (val: number) => setVisibleApply(visibleApplyButton && val > 0)
  ...
  React.useEffect(() => {
    ...
    setNewOptions(pickBy({
      ...options,
      ...newOptions
    },(_, label) => keys.includes(label)))
  },[options])

  const onOptionClick: IOnOptionClick = label => ({ target }) => {
    const { checked } = (target as HTMLInputElement)
    setNewOptions(listHandlers[listType](label, checked))
    !!optionClickHandler && optionClickHandler(label)(null)
  }

  const showSearch = !!onSearch || (!!showSearchFor && Object.keys(options).length >= showSearchFor)
  return (
    <div data-sk-name="sked-dropdownlist" className={ classnames('sked-dropdownlist max-w-xs sk-mx-1', classNames) }>
      { showSearch &&
          <SearchItem onSearch={ searchHandler } searchPlaceholder={ searchPlaceholder } />
      }
      { showList &&
        <>
...
export const SearchOnlyFilter: React.FC<DropdownListProps> = props => {
  ...
  const [filteredOptions, setFilteredOptions] = React.useState(pickSelected(props.options as Options))
  const [search, setSearch] = React.useState('')
  const onSearch = ({ target: { value } }: { target: { value: string } }) => setSearch(value.toLowerCase())
...
const filterAndSort = (allOptions: Options, search: string) => {
  const filtered = pickBy(allOptions,
    (_, label) => label.toLowerCase().includes(search)
  )
  const sorted = sortBy(Object.keys(filtered), label => label.toLowerCase().indexOf(search))
...
const listHandlers: ListHandlers  =  {
  checkbox: (label, value) => prevOptions => ({ ...prevOptions, [label]: value }),
...
const ListRenderer: React.FC<{
  ...
  const items = searchValue
    ? pickBy(options,
        (_, label) => label.toLowerCase().includes(searchValue.toLowerCase())
      )
    : options
...
const ArrayOptionRenderer: React.FC<ArrayOptionRendererProps> = ({
  ...
  const [searchValue, setSearchValue] = React.useState('')
  const searchHandler = onSearch || (({ target: { value } }: { target: { value: string } }) => setSearchValue(value.toLowerCase()))
  const optionsToDisplay = searchValue
    ? options.filter(option => option.toLowerCase().includes(searchValue))
    : options
  const showSearch = !!onSearch || (!!showSearchFor && options.length >= showSearchFor)
  return (
  <div data-sk-name="sked-dropdownlist" className="sked-dropdownlist sk-mx-1">
    ...
    <ul className="sked-dropdownlist-list" >
      { optionsToDisplay.length ? optionsToDisplay.map((option, index) => (
        <DropdownRow
        ...
        >
          <div className="sk-flex sk-justify-between sk-items-center">
            { option }{ !!selectedCount[option] && <Badge className="sk-ml-2" count={ selectedCount[option] } /> }
          </div>
        </DropdownRow>
      )) : <DefaultEmptyState>{ emptyState }</DefaultEmptyState> }
    </ul>
...
const SearchItem: React.FC<Pick<DropdownListProps, 'onSearch' | 'searchPlaceholder'>> = ({ onSearch, searchPlaceholder }) => {
...
const DefaultEmptyState: React.FC = ({ children }) => (
  <div data-sk-name="sk-filter-empty-state" className="sk-text-grey sk-px-3 sk-py-3 sk-flex sk-justify-start sk-items-center sk-text-sm">
    { children }
  </div>
)
