...
const getTooltipContent = (part?: GraphPart) => {
  return (
    <span>
      {part ? (
        <>
          <span style={{ color: part.color }}>&bull; </span>
          {`${part.title} ${Math.round(part.value)}%`}
        </>
      ) : (
        "No data"
      )}
...
const COLORS = ["#007ee6", "#ffc878", "#75c8f6", "#3da499", "#8d68c3", "#000"];
...
const prepareWorkloadData = (workload: { [index: string]: number }) => {
  ...
  if (parts.length > MAX_MEANINGFUL_PARTS) {
    const other = parts.slice(MAX_MEANINGFUL_PARTS - 1).reduce((acc, part) => ({
      ...acc,
      title: "Other",
    ...
    return [...parts.slice(0, MAX_MEANINGFUL_PARTS - 1), other];
  }
...
const renderLegend = (parts: GraphPart[]) => {
  return (
    <div className="sk-text-xs sk-capitalize workload__legend">
      {parts.map((part, idx) => (
        <div className="sk-mb-2 sk-leading-none" key={idx}>
          <span className="sk-text-2xl sk-pr-2" style={{ color: part.color }}>
            &bull;
          </span>
...
const renderLegendPlaceholder = () => <div className="sk-mt-8">No data</div>;
