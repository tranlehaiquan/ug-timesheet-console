...
export const Badge: React.SFC<IProps> = ({ count = 0, countLimiter = 99, badgeType = 'default', className }) => {
  const checkedCount = Math.max(0, count)
  const checkedCountLimiter = Math.max(1, countLimiter)
  const countText = checkedCount > checkedCountLimiter ? `${checkedCountLimiter}+` : `${checkedCount}`
