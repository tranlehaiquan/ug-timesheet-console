...
import { getPositionEvalOrder, getPositionScores, getBestFit, adjustContentDisplayCoordinates, IAnchorScores, notTargetAreas, ICursorOptions, ICursorPoints, Position } from './info-window-utils'
...
export class InfoWindow extends React.PureComponent<IInfoWindowProps, IState> {
  ...
  componentWillReceiveProps(newProps: IInfoWindowProps, oldProps: IInfoWindowProps) {
  ...
  setShow = (show: boolean) => {
    ...
    const mountContent = () => {
      ...
      if (delayShow > 0 && isOpening) {
        this._showDelayId = setTimeout(() => addListenersThenRender(), delayShow)
      } else {
        addListenersThenRender()
      }
    }
    const postAction = onClose && isClosing ? () => { mountContent(); onClose() } : mountContent
  ...
  handleBodyClick = (e: MouseEvent) => {
    if ((this.props.event === 'click' || this.props.event === 'mount') && this.state.show) {
  ...
  handleScroll = (e: Event) => {
    const updatedTriggerRect = this._triggerRef ? this._triggerRef.getBoundingClientRect() : null
    const pageScrolled = this._triggerRect.top !== updatedTriggerRect.top || this._triggerRect.left !== updatedTriggerRect.left
  ...
  getDisplayPosition = (triggerRect: ClientRect, triangleRect: ClientRect, contentRect: ClientRect) => {
    ...
    const positionScores = getPositionScores(triggerPoint, triggerRectScrollShifted, contentRect, triangleRect, windowRect, cursorOption, this.props.positionOffset)
  ...
  computeStyles() {
    ...
    const newPosition = this.getDisplayPosition(this._triggerRect, triangleRect, this._contentRef.getBoundingClientRect())
    const styles = this.computeStylesFromDisplayPosition(newPosition, triangleRect)

    return { ...mapValues(styles, container => mapValues(container, value => `${value}px`)), position: newPosition.position }
  }
  ...
  renderContent(runStyleRender: boolean = true) {
    ...
    if (show) {
      ...
      const renderContainer = this._renderContainer || this.createRenderContainer()
      render(content, renderContainer, () => runStyleRender && this.renderContent(false))
    } else {
      this.removeRenderingContainer()
    }
  }
  ...
  render() {
    ...
    const updatedStyle = displayInline ? { ...style, display: 'inline-block' } : style
