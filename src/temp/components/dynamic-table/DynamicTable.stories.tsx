...
export const data: DataTypes[] = [
  {
    description:
      "Sucrose is rapidly lost during the roasting process, and may disappear entirely in darker roasts. During roasting, aromatic oils and acids weaken, changing the flavor; at 205 °C (401 °F), other oils start to develop.",
    account: "Coffee Shop",
  ...
  {
    description:
      "Ford has competed in rallycross with its Ford Fiesta and Ford Focus.",
  ...
  {
    description:
      "An F430 Spider that runs on ethanol was displayed at the 2008 Detroit Auto Show.",
    account: "Auto Workshop",
    contact: null,
    jobType: "Break Fix",
    scheduled: "Friday 10 Aug 9 - 11am",
  ...
  {
    description:
      "The first cafeteria in Vienna was founded in 1683 by a Ukrainian cossack and Polish diplomat of Ruthenian descent, Jerzy Franciszek Kulczycki, who was also the first to serve coffee with milk.",
    account: "Coffee Shop",
    contact: "Maxwell House",
    jobType: "Installation",
    scheduled: null,
  ...
  {
    description:
      "The espresso bar is a type of coffeehouse that specializes in coffee drinks made from espresso. Originating in Italy, the espresso bar has spread throughout the world in various forms.",
    account: "Coffee Shop",
...
storiesOf("Dynamic table", module)
  ...
  .add("Column options", () => {
    ...
    return <DynamicTable data={data} config={optionsConfig} />;
  })
  .add("With cell renderer", () => {
    const rendererConfig: TableConfig<DataTypes> = {
      ...
      columns: [
        ...
        } as TableConfigColumns<DataTypes, "status">,
        {
          ...
          cellRenderer: (data) =>
            data.map((r, index) => <Avatar key={index} name={r} />),
        ...
        },
        {
          ...
          cellRenderer: (data, row, index, rowIndex) => (
            <div>
              <div>
                Data: {data}; CellIndex: {index}; RowIndex: {rowIndex};
              </div>
              <div>
                {Object.keys(row)
                  .slice(0, 3)
    ...
    return <DynamicTable data={data} config={rendererConfig} />;
  })
    ...
    return (
      <SortingWrapper
        ...
        render={
          ...
          (sortedData, configWithHandler) => (
            <DynamicTable data={sortedData} config={configWithHandler} />
          )
        }
      />
    );
  ...
  .add("With select", () => {
    ...
    const selectOpts = validKeys.reduce(
      (acc, opt) => ({ ...acc, [opt]: opt }),
      {}
    );
  ...
  .add("Resizable columns", () => {
    ...
    return <DynamicTable data={data} config={config} />;
  })
  ...
  .add("With grouping", () => {
    ...
    const groupKey = select(
      "groupBy",
      ["jobType", "account", "contact", "resources"],
      "jobType"
    );
    let groupedData = [];
    ...
    return (
      <DynamicTable data={data} config={config} groupedData={groupedData} />
    );
  });
