...
export class Avatar extends React.PureComponent<IAvatar, {}> {
  ...
  getInitials(name: string = '') {
    if (name.length > 1) {
      ...
      return (first && last ? `${first.charAt(0)}${last.charAt(0)}` : first.substring(0, 2)).toUpperCase()
    }
  ...
  wrapInTooltip(avatar: JSX.Element) {
    return (
      <Tooltip content={ this.props.tooltipText || this.props.name } position={ 'top' } delayShow={ 250 }>
        { avatar }
      </Tooltip>
    )
  }
  ...
  renderAvatar() {
    ...
    return (
      <div className={ classnames('sked-avatar', className, this.sizeClassMap[size]) } { ...otherProps }>
        { imageUrl ? <img className="sked-avatar__image" src={ imageUrl } /> : this.getInitials(name) }
      </div>
    )
  }

  render() {
    return this.props.showTooltip ? this.wrapInTooltip(this.renderAvatar()) : this.renderAvatar()
  }
}
