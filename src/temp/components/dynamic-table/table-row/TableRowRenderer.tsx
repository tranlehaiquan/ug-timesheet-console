...
function TableRowRendererComponent<T>({
  rowData,
  config: { columns, options, condensed },
  className,
  rowIndex,
  onSelect,
  selectChecked,
  ...
  */
    ...
    return (
      <TableCell className={classnames({ "sked-fixed-cell-row": fixedColumn })}>
        <FormInputElement
          ...
          onChange={onSelect(rowData[selectBy])}
  ...
  const expandPlaceholderRenderer = () => {
    return <TableCell className="sk-text-right sk-pl-0 sk-pr-6" />;
  };
    ...
    <>
      <TableRow
        className={classnames(
          "sked-table-row",
          { "sked-table-row--selected": selectChecked },
          className
        )}
      ...
      >
        ...
        {options &&
          !options.selectable &&
          selectPlaceholder &&
          selectPlaceholderRenderer()}
        {columns.map(
          (
            { key, cellRenderer, cellClasses, emptyPlaceholderText, width },
            index
          ) => (
            <TableCell
              key={`${rowIndex}-${index}`}
              className={classnames(cellClasses || "", {
                ...
                "sked-fixed-cell-row-selectable":
                  fixedColumn && index === 0 && options && options.selectable,
              })}
              style={{
            ...
            >
              {get(rowData, key) !== null &&
              get(rowData, key) !== undefined &&
              cellRenderer
...
const ExpandableRow: React.FC<{
  ...
  rowData: object;
}> = ({ isExpanded, selectable = false, columnsNumber, onExpand, rowData }) => {
  ...
  return (
    <TableRow style={{ borderBottomWidth: isExpanded ? 1 : 0 }}>
      ...
      <td colSpan={columnsNumber} className="sk-p-0">
        ...
        >
          <div className="sk-px-5 sk-pb-6">{onExpand && onExpand(rowData)}</div>
        </div>
      </td>
      <td className="sk-p-0" />
    </TableRow>
...
export const TableRowRenderer: React.NamedExoticComponent<Props<any>> =
  React.memo(TableRowRendererComponent, differentNameProps);

function defaultRenderer<T extends React.ReactNode>(
  data: T,
  key: string,
  emptyText: string,
  condensed: boolean = false
): React.ReactNode {
  const content: string | React.ReactNode = data || emptyText || (
    <div className="sk-text-grey">{`No ${key} data`}</div>
  );
  return condensed ? <Condenser>{content}</Condenser> : content;
}

function differentNameProps<T>(
  prevProps: Props<T>,
  nextProps: Props<T>
): boolean {
