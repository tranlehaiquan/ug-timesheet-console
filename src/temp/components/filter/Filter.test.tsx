...
describe("Filter", () => {
  ...
  it("renders dropdown on click", () => {
    ...
    expect(
      wrapper.find('[data-sk-name="sk-filter-dropdown-row"]')
    ).toHaveLength(wrapper.prop("filters").length);

    wrapper
      .find('[data-sk-name="sk-filter-dropdown-row"]')
      .at(0)
      .simulate("click");
    expect(
      wrapper.find('[data-sk-name="sk-filter-dropdown-row"]')
    ).toHaveLength(wrapper.prop("filters")[0].options.length);
  });
  ...
  it("adds and removes filters", () => {
    ...
    const wrapper = mount(
      <Filter
        {...getDefaultTestProps((activeFilters) => {
          callbackValue = activeFilters;
        })}
      />
    );

    const apply = () =>
      wrapper.find('[data-sk-name="sk-filter-apply"]').at(0).simulate("click");
    ...
    wrapper
      .find('[data-sk-name="sk-sk-added-filter-handler"]')
      .simulate("click");
  ...
  it("filters options with search", () => {
    ...
    wrapper
      ...
      .simulate("click");
    wrapper
      .find('[data-sk-name="sk-filter-search-input"]')
      .find("input")
    ...
    wrapper
      .find('[data-sk-name="sk-filter-search-input"]')
      .at(0)
      .simulate("change", { target: { value: "option1" } });
    expect(
      wrapper.find('[data-sk-name="sk-filter-dropdown-row"]')
    ).toHaveLength(1);
