...
interface Props {
  ...
  customSubheader?: (data: { day: Date, dayEvents: CalendarEvent[], allEvents: CalendarEvent[] }) => React.ReactElement
}

const defaultSubheader = (data: { day: Date, dayEvents: CalendarEvent[], allEvents: CalendarEvent[] }) => {
  return (
    <p>{ data.day.toISOString() }</p>
  )
}

const daysVisible = 7

export const Calendar: React.FC<Props> = ({ startDate, endDate, events, customSubheader }) => {
  ...
  React.useLayoutEffect(() => {
    ...
    const currentScrollWidth = scrollMeasurer.offsetWidth - scrollMeasurer.clientWidth
    setScrollWidth(currentScrollWidth)
    document.body.removeChild(scrollMeasurer)
    setColumnWidth((ref.current!.offsetWidth - 60 - currentScrollWidth) / daysVisible)
