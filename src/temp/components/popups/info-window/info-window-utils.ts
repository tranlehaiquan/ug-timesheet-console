...
export const getAnchorPoints = (triggerPoint: ICursorPoints, triggerRect: ClientRect, triangleRect: ClientRect, windowRect: ClientRect, cursorOptions: ICursorOptions): IAnchorPoint[] => {
  const evalHorizontalAxisPosition = (position: Position) => {
    // closest point to the trigger point if we're in follow mode
    if (cursorOptions && (cursorOptions.rule === 'follow' || cursorOptions.rule === 'follow-x')) {
      return {
        x: triggerPoint.x,
        y: cursorOptions.rule === 'follow' ? triggerPoint.y : triggerRect[position]
      }
    }
  ...
  const evalVerticalAxisPosition = (position: Position) => {
    // closest point to the trigger point if we're in follow mode
    if (cursorOptions && (cursorOptions.rule === 'follow' || cursorOptions.rule === 'follow-y')) {
      return {
        x: cursorOptions.rule === 'follow' ? triggerPoint.x : triggerRect[position],
        y: triggerPoint.y
      }
    }
  ...
  return [
    ...
    { position: 'bottom' as Position, point: evalHorizontalAxisPosition('bottom') },
...
export const getPositionScores = (
  ...
  const anchorPoints = getAnchorPoints(triggerPoint, triggerRect, triangleRect, windowRect, cursorOptions).map(applyAnchorOffset(anchorOffset))

  const displays = anchorPoints.map(anchor => {
    const displayCoords = getDisplayCoordinates(anchor, contentRect, triangleRect)
    return {
      ...anchor,
      display: {
        ...displayCoords,
  ...
  const distanceScores = displayScores.map(anchor => {
    const distanceFromAnchor = Math.sqrt(Math.pow(anchor.point.x - triggerPoint.x, 2) + Math.pow(anchor.point.y - triggerPoint.y, 2))
  ...
  return distanceScores
    ...
    .map(anchor => {
      return {
        ...anchor,
        weightedScore: anchor.weightedDisplayScore - anchor.weightedDistanceScore
      }
    })
}

export const getBestFit = (desiredOrder: Position[], positionScores: IAnchorScores[]): IAnchorScores => {
  const startingDisplayThreshold = 1

  const bestPositions = scanAnchorsForBestFit(positionScores, startingDisplayThreshold)

  if (bestPositions.length === 0) {
    return positionScores[0]
  }

  const bestPositionsInDesiredOrder = compact(desiredOrder.map(position => bestPositions.find(anchor => anchor.position === position)))

  return bestPositionsInDesiredOrder[0]
}

export const adjustContentDisplayCoordinates = (anchor: IAnchorScores, windowRect: ClientRect) => {
  const intersection = calculateIntersectionArea(anchor.display, windowRect)

  if (anchor.position === 'top' || anchor.position === 'bottom') {
    const moveX = anchor.display.right > intersection.right ? (anchor.display.right - intersection.right) * -1 : intersection.left - anchor.display.left
  ...
  {
    const moveY = anchor.display.bottom > intersection.bottom ? (anchor.display.bottom - intersection.bottom) * -1 : intersection.top - anchor.display.top
...
const getWeightedDistanceScore = (distance: number, threshold: number = 100, currentScore: number = 0.05): number => {
  if (currentScore >= 1) {
    return 1
  }
  ...
  return getWeightedDistanceScore(distance, threshold * 1.5, currentScore * 1.75)
}

const calculateIntersectionArea = (innerRect: ClientRect, outerRect: ClientRect) => {
  // const SPACER_UNIT = 6
...
const scanAnchorsForBestFit = (positionScores: IAnchorScores[], threshold: number, rangeIncriment: number = 0.05): IAnchorScores[] => {
  if (threshold <= -1) {
    return []
  }

  const scoresPassing = positionScores.filter(position => position.weightedScore > threshold)

  if (scoresPassing.length > 0) {
    return scoresPassing
  }
  return scanAnchorsForBestFit(positionScores, threshold - rangeIncriment, rangeIncriment * 1.5)
}

const getDisplayCoordinates = (anchor: IAnchorPoint, contentRect: ClientRect, triangleRect: ClientRect) => {
