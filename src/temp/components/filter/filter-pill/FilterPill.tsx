...
export const FilterPill: React.RefForwardingComponent<HTMLDivElement, FilterPillProps & React.RefAttributes<HTMLDivElement>>  = React.forwardRef(
  ({
  name,
  selected = [],
  isActive = false,
  ...
  classNames
    ...
    React.useEffect(() => {
      if (labelsRef.current){
        ...
        const buttonWidth = element.nextElementSibling ? nextElement.offsetWidth : 0
        const isOverflow = element.clientWidth + buttonWidth > parentElement.offsetWidth
    ...
    <div
      ...
      className={
        classnames('sk-flex sk-flex-no-wrap sk-whitespace-no-wrap sk-max-w-xs sk-border sk-border-solid sk-rounded sk-items-center hover:sk-bg-blue-lightest', {
    ...
    >
      <div
        ...
        onClick={ onClick }
        className={
          classnames('sk-px-3 sk-py-2 sk-m-0 sk-border-0 sk-max-h-4 sk-max-w-full sk-flex sk-flex-no-wrap sk-items-baseline', { 'sk-pr-0': !fixed && !!onRemove })
        }
      >
        { name && (<NameRenderer name={ name } />) }
        { !!selected.length &&
          <SelectionRenderer selected={ selected } className={ name ? 'sk-ml-1' : '' } />
        }
      </div>
      {
        !fixed && onRemove &&
...
const selectionClassNames = 'sk-text-sm sk-font-medium sk-text-navy sk-truncate sk-max-w-full'

const SelectionRenderer: React.FC<{selected: string[], className: string}> = ({ selected, className }) => {
  return (
    selected.length !== 2
    ? (
        <div className={ classnames(selectionClassNames, className) }>
          { selected.length > 2 ? `${selected.length} selected` : selected.join(', ') }
        </div>
      )
    : <SelectionRendererEdge selected={ selected } className={ className } />
  )
}
const SelectionRendererEdge: React.FC<{selected: string[], className: string}> = ({ selected, className }) => {
    ...
    <div className={ classnames(selectionClassNames, className) } ref={ ref }>
      {
        typeof firstTruncated === 'undefined'
          ? `${selected[0]}, .......`
          : firstTruncated ? `${selected.length} selected` : selected.join(', ')
      }
    </div>
  )
}
