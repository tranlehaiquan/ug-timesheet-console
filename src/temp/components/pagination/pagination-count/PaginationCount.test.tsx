...
describe("PaginationCount", () => {
  it("renders Pagination details", () => {
    ...
    const PaginationCountText = wrapper
      .find('[data-sk-name="pagination-count-container"]')
      .getDOMNode().innerHTML;

    expect(wrapper.find(PaginationCount).length).toBe(1);
    expect(
      wrapper.find('[data-sk-name="pagination-count-container"]').length
    ).toBe(1);

    const paginationText = `${(currentPage - 1) * itemsPerPage + 1}-${
