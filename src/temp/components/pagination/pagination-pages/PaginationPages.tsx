...
export const PaginationPages: React.FC<IProps> = (props) => {
  ...
  const truncateLimiter =
    props.truncateLimiter < 1 || null ? 1 : props.truncateLimiter;
  const currentPageDelta =
    props.currentPageDelta < 1 || null ? 1 : props.currentPageDelta;
  const getPages = renderPaginationItems(
    itemsTotal,
    itemsPerPage,
    currentPage,
    currentPageDelta,
  ...
  const changeHandler = (newPage: number) => (event: React.MouseEvent) =>
    newPage !== currentPage ? onPageChange(newPage) : null;
    ...
    >
      <div
        className={classnames(
          ...
          {
            "sk-text-grey-light sk-cursor-not-allowed sk-bg-white hover:sk-bg-white":
        ...
        page !== "..." ? (
          <div
            className={classnames(
              ...
              {
                "sk-bg-navy-lighter sk-text-white hover:sk-bg-navy-lighter sk-text-white":
                  page === currentPage,
              },
            ...
            data-sk-name={
              page === currentPage
                ? "pagination-pages-item-number-current"
                : "pagination-pages-item-number"
            }
          >
            {page}
          </div>
          ...
          <div
            className={
              "sked-pagination-item-min-w sk-w-auto sk-h-6 sk-px-2 sk-flex sk-items-center sk-justify-center sk-rounded sk-cursor-default sk-select-none sk-text-sm sk-leading-normal"
            }
            key={index}
