...
import { ActionBar, ActionBarActions, ActionBarViewControls, IActionBar } from './ActionBar'
...
describe('ActionBar', () => {
  ...
  it('renders action bar with content', () => {
    ...
    const wrapper = mount(
      <ActionBar { ...getDefaultTestProps() }>
        <ActionBarActions>
          { range(itemsCount).map(index => <div key={ index }>action{ index + 1 }</div>) }
        </ActionBarActions>
        <ActionBarViewControls withSearch={ false }>
          { range(itemsCount).map(index => <div key={ index }>control{ index + 1 }</div>) }
  ...
  it('renders action bar with view controls items with search', () => {
    ...
    expect(wrapper.find(viewControls).length).toEqual(1)
    expect(wrapper.find(viewControls).children().length).toEqual(itemsCount + 1)
  })
