...
interface ITimesheetEntriesGroupRows {
  ...
  columns: {
    ...
    className?: string
    cellRenderer?: (entry: ReduxDataTypes.TimesheetEntry, isFirst?: boolean) => React.ReactElement
  }[]
}

const TimesheetEntriesGroupRows: React.FC<ITimesheetEntriesGroupRows> = ({ entries, columns }) => {
  return (
    <>
          ...
          <tr key={ entryIndex } className="sk-table-row">
            {
              columns.map((column, colIndex) => {
                ...
                return (
                  <td
                    key={ colIndex }
                    className={ classnames('sk-py-3 sk-px-5 sk-text-xs sk-border-grey-lighter', column.className, {
                      'sk-border-b': isLast
                    }) }
                  >
                    {
                      column.cellRenderer
                        ...
                        : isNil(entry[column.key]) ? '-' : entry[column.key]
                    }
                  </td>
                )
              })
...
const getNameOrDisplayName = (entry: ReduxDataTypes.TimesheetEntry, suffix: string) => {
...
export const TimesheetEntriesTable: React.FC<ITimesheetEntriesTable> = ({ timesheet }) => {
  const [updateTimesheetEntryId, setUpdateTimesheetEntryId] = React.useState<string | undefined>(undefined)
  const [deleteTimesheetEntryId, setDeleteTimesheetEntryId] = React.useState<string | undefined>(undefined)
  const { defaultTimezone } = useSelector<ReduxDataTypes.State, ReduxDataTypes.Settings>(state => state.settings)
  ...
  const entriesByDay = groupBy(timesheetEntriesTableItems, entry => entry.StartDate)
  ...
  const columns = [
    ...
    },
    {
      ...
      name: 'Start',
      cellRenderer: (entry: ReduxDataTypes.TimesheetEntry) => (
        <DateTimeCell date={ entry.StartDate } time={ entry.StartTime } timezone={ entry.Timezone || defaultTimezone } format="D MMM YYYY h:mm a (z)" />
      )
    },
    {
      ...
      name: 'End',
      cellRenderer: (entry: ReduxDataTypes.TimesheetEntry) => (
        <DateTimeCell date={ entry.EndDate } time={ entry.EndTime } timezone={ entry.Timezone || defaultTimezone } format="D MMM YYYY h:mm a (z)" />
      )
    },
    {
      key: 'Duration' as keyof ReduxDataTypes.TimesheetEntry,
      cellRenderer: (entry: ReduxDataTypes.TimesheetEntry) => (
        <DurationCell value={ typeof entry.Duration === 'number' ? entry.Duration : undefined } isInMinutes />
      )
    },
    {
      key: 'Lunch Break' as keyof ReduxDataTypes.TimesheetEntry,
      cellRenderer: (entry: ReduxDataTypes.TimesheetEntry) => (
        <DurationCell value={ typeof entry.LunchBreakDuration === 'number' ? entry.LunchBreakDuration : undefined } isInMinutes />
      )
    },
    {
      key: 'Actual Duration' as keyof ReduxDataTypes.TimesheetEntry,
      cellRenderer: (entry: ReduxDataTypes.TimesheetEntry) => (
        <DurationCell value={ typeof entry.ActualDuration === 'number' ? entry.ActualDuration : undefined } isInMinutes />
      )
    },
    {
    ...
    },
    {
      ...
      cellRenderer: (entry: ReduxDataTypes.TimesheetEntry) => (
        <NumberCell value={ entry.Distance! % 1 ? entry.Distance!.toFixed(2) : entry.Distance } unit={ entry.DistanceUnit } />
      )
    },
    {
          ...
          <tr className="sk-h-10 sk-text-left">
            { columns.map((column, index) => {
              return (
                <th
                  key={ index }
                  className={ classnames('sk-px-5 sk-font-medium sk-table-header-cell', column.className) }
