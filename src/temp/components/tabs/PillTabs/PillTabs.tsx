...
export class PillTabs extends React.PureComponent<
  IPillTabsProps,
  IPillTabsState
> {
  state = {
  ...
  render() {
    ...
    const tabClasses =
      "sk-mr-4 sk-px-3 sk-py-1 sk-uppercase sk-rounded sk-text-white sk-cursor-pointer sk-font-semibold sk-tracking-wider sk-opacity-75 hover:sk-opacity-100 hover:sk-bg-navy-light hover:sk-shadow-tabs";

    return (
      <ul {...otherProps} className={classes}>
        {tabs.map((tab) => {
          const combinedTabClasses = classNames(tabClasses, tabClassName, {
            [`${activeClasses} ${
