...
export const calculateStats = (timesheetData: ReduxDataTypes.TimesheetData[]) => {
  const summary = {
    jobs: 0,
    distance: 0,
    logged: 0,
...
const dateFilter = (startDate: string, endDate: string) => `StartDate >= ${startDate} AND EndDate <= ${endDate}`
...
export const getTimesheets = makeAsyncActionCreatorSimp(
  TIMESHEET, () => async (dispatch: Dispatch, getState: () => ReduxDataTypes.State) => {
    const store = getState()
    const filters = createAllTimesheetsFilters(store)
    try {
      const timesheet = await autoPaginationGraphqlRequest<ReduxDataTypes.Timesheet>(
        Queries.AllTimesheetsQuery(store.vendor), 'timesheet', {
          filters
        }, ['UID']
      )
      return { timesheet }
      ...
      if (errs && store.settings.showErrorMessage) {
        const messages = (errs as {message: string}[]).map((err: {message: string}) => err.message).join(', ')
...
const calculateMissingFields = (timesheet: ReduxDataTypes.TimesheetSummaryFields, entry: ReduxDataTypes.TimesheetEntry) => {
  ...
  const startDate = moment(`${entry.StartDate} ${entry.StartTime}`, 'YYYY-MM-DD HH:mm')
  const endDate = moment(`${entry.EndDate} ${entry.EndTime}`, 'YYYY-MM-DD HH:mm')

  const actualDuration = entry.ActualDuration ? entry.ActualDuration / 60 : 0
  const duration = actualDuration || Math.round(differenceInMinutes(endDate.toDate(), startDate.toDate()) / 60 * 100) / 100
...
export const reduceTimesheetEntriesData = (timesheetElement: ReduxDataTypes.TimesheetData) => {
  const missingFields = timesheetElement.Entries.reduce((acc, entry) => calculateMissingFields(acc, entry), {
    Activity: null,
    Distance: null,
    Logged: null,
    Unavailability: null
...
export const formatEntryData = (entry: ReduxDataTypes.TimesheetEntry) => {
  const duration = calculateDurationInMinutes(`${entry.StartDate}T${entry.StartTime}Z`, `${entry.EndDate}T${entry.EndTime}Z`)
  const actualDuration = entry.ActualDuration || duration
  // const lunchBreak = duration - actualDuration
  const jobAllocation = entry.Job ? entry.Job.JobAllocations.find(ja => ja.ResourceId === entry.Timesheet.ResourceId) : undefined
...
const addJobAllocationId = (timesheet: ReduxDataTypes.TimesheetData) => {
  const JobAllocationIds = timesheet.Entries.filter(entry => !!formatEntryData(entry).JobAllocationId).map(entry => {
...
export const timesheetsReducers = {
  ...
  [CLEAR_TIMESHEET_DATA]: (state: ReduxDataTypes.State) => ({ ...state, timesheetData: ([] as ReduxDataTypes.TimesheetData[]) })
}
