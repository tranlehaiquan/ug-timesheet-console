...
export const PaginationCount: React.SFC<IProps> = React.memo(props => {
  ...
  return (
    <div
      className={ 'sked-pagination-count-min-w sk-flex-grow-0 sk-flex sk-items-center sk-ml-6 sk-text-sm sk-text-navy-light' }
...
const getItemsSpan = (itemsPerPage: number, itemsTotal: number, currentPage: number) => {
  ...
  const itemsSpanEnd = currentPage !== totalPages ? currentPage * itemsPerPage : itemsTotal
  const itemsSpan = itemsSpanStart === itemsSpanEnd ? itemsSpanStart : `${itemsSpanStart}-${itemsSpanEnd}`
