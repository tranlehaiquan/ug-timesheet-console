...
export const ResponsiveDropdown: React.FC<{
  children: (modifiers: NonNullable<PopperProps['modifiers']>, placement: NonNullable<PopperProps['placement']>) => React.ReactElement
}> = ({ children }) => {
  return (
    children({ ...modifiers,   preventOverflow: { enabled: true, fn: updateFunc } }, placement)
  )
}
...
const updateFunc = (data: any) => {
  ...
  const { bottom: triggerBottom, top: triggerTop } = data.instance.reference.getBoundingClientRect()
  ...
  if (wrapper) {
    ...
    if ((elementHeight - outBot) <  46) {
      data.instance.modifiers.find(({ name }: {name: string}) => name === 'flip').enabled = true
      wrapper.style['max-height'] = `${Math.min(Math.max(elementHeight - outTop, 46) - 10, MAXHEIGHT)}px`
    } else {
      data.instance.modifiers.find(({ name }: {name: string}) => name === 'flip').enabled = false
      wrapper.style['max-height'] = `${Math.min(Math.max(elementHeight - outBot, 46) - 10, MAXHEIGHT)}px`
    }
