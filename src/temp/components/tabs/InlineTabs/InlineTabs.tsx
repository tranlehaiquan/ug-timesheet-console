...
export const getInitialSubmenuItem = ({
  initialActiveSubmenuItem,
  initialActiveTab,
  tabs,
}: Pick<
  IProps,
  ...
  if (initialActiveTab && initialActiveTab.submenu) {
    const item = initialActiveSubmenuItem
      ? initialActiveTab.submenu.find(
          (a) => a.id.toLowerCase() === initialActiveSubmenuItem.toLowerCase()
        )
      : null;
...
export class InlineTabs extends React.PureComponent<IProps, IState> {
  state: IState = {
    activeTab: this.props.initialActiveTab
      ? this.props.initialActiveTab.id
  ...
  createRefs = (refId: InlineTab["id"]) => (ref: HTMLLIElement) =>
    (this.tabs[refId] = ref);

  handleTabClick =
    (tabClickedId: InlineTab["id"], submenuClickedId?: SubmenuItem["id"]) =>
    () => {
      ...
      if (this.tabs[tabClickedId]) {
        ...
        const activeSubTabToSet = tabs.find((tab) => tab.id === tabClickedId)
          .submenu[0].id;
        this.setState({
          activeTab: tabClickedId,
          activeSubmenuItem: activeSubTabToSet,
          submenuIsActive: false,
        });

        onTabClick({
          tabClickedId,
          submenuClickedId: submenuClickedId || activeSubTabToSet,
        });
      } else {
        this.setState({
          activeTab: tabClickedId,
  ...
  handleSubmenuItemClick = (submenuClickedId: SubmenuItem["id"]) => () => {
    ...
    this.setState({
      activeSubmenuItem: submenuClickedId,
      activeTab: activeSubmenuParentId,
      activeSubmenuParentId: null,
      submenuIsActive: false,
    });
    ...
    this.props.onTabClick({
      submenuClickedId,
      tabClickedId: activeSubmenuParentId,
    });
  };

  handleMouseHover =
    (tabSubMenu: SubmenuItem[], tabId: InlineTab["id"]) => () => {
      ...
      if (activeSubmenuRef) {
        this.setState({
          ...
          submenuItems: tabSubMenu,
          activeSubmenuParentId: tabId,
        });
      }
    };

  handleParentMouseLeave =
    (tabId: InlineTab["id"]) => (evt: React.MouseEvent<HTMLLIElement>) => {
      ...
      if (submenuRef) {
        ...
        if (
          tabRect.top > evt.clientY ||
          evt.clientX < tabRect.left ||
          evt.clientX > tabRect.right
        ) {
  ...
  render() {
    ...
    const { tabs, className, tabClassName, activeTabClassName, ...otherProps } =
      this.props;
    ...
    const defaultTabClasses =
      "sk-cursor-pointer sk-mx-3 sk-px-1 sk-pb-2 sk-inline-block sk-whitespace-no-wrap sk-leading-tight";
    const activeTabDefaultClasses =
      "sk-border-b-2 sk-border-navy sk--mb-px sk-text-navy sk-font-semibold";
    const tabHoverClasses = "hover:sk-text-navy";

    const newProps = omit(otherProps, [
      "onTabClick",
      "initialActiveTab",
      "tabIconClassName",
      "initialActiveSubmenuItem",
