...
const getEntryEventName = (entry: ReduxDataTypes.TimesheetEntry) => {
  switch (entry.EntryType) {
    case "Unavailability":
      return entry.Unavailability
        ? `${entry.Unavailability.Type}`
        : entry.EntryType;
    case "Activity":
      return entry.Activity
        ? `${entry.Activity.Name} - ${entry.Activity.Type}`
        : entry.EntryType;
    case "Job":
      return entry.Job
        ? `${entry.Job.Name} - ${entry.Job.Type}`
        : entry.EntryType;
    case "Shift":
...
const eventThemes = {
  ...
  [EventType.JobPendingAllocation]: {
    backgroundColor: "#f3f5f9",
    fontColor: "#4e5b78",
    borderColor: "#4e5b78",
  },
  [EventType.JobPendingDispatch]: {
