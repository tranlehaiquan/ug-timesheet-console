...
const submenu = [{ name: 'Sub one', id: 'sub-one' }, { name: 'Sub two', id: 'sub-two' }]
...
describe('InlineTabs', () => {
  ...
  test('renders with correct initial state', () => {
    // Arrange
    const component = shallow<{}, IState>(<InlineTabs tabs={ tabs } onTabClick={ jest.fn() } />)
  ...
  test('renders with correct state when passed a initialActiveTab', () => {
    // Arrange
    const component = shallow<{}, IState>(<InlineTabs tabs={ tabs } initialActiveTab={ tabs[2] } onTabClick={ jest.fn() } />)
  ...
  test('clicking a new tab sets the correct state', () => {
    ...
    const component = shallow<{}, IState>(<InlineTabs tabs={ tabs } onTabClick={ onTabClickSpy } />)
    ...
    const tabClickPayload = { tabClickedId: 'tab-three', submenuClickedId: null }
  ...
  describe('getInitialSubmenuItem', () => {
    test('no initialActiveTab passed', () => {
      // Arrange/Act
      const value = getInitialSubmenuItem({ tabs, initialActiveTab: null, initialActiveSubmenuItem: null })
    ...
    test('initialActiveTab passed through', () => {
      ...
      // Act
      const value = getInitialSubmenuItem({ tabs, initialActiveTab, initialActiveSubmenuItem: null })
    ...
    test('initialActiveSubmenuItem passed through with NO initialActiveTab', () => {
      // Arrange/Act
      const value = getInitialSubmenuItem({ tabs, initialActiveTab: null, initialActiveSubmenuItem: submenu[1].id })
    ...
    test('initialActiveSubmenuItem passed through with initialActiveTab', () => {
      ...
      // Act
      const value = getInitialSubmenuItem({ tabs, initialActiveTab, initialActiveSubmenuItem: submenu[1].id })
