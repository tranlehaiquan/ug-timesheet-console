...
const defaultColumns = (num = 1, options = {}) => ({
  columns: Array.from(Array(num)).map(() => ({ key: "test", ...options })),
});
...
describe("TableRowRenderer", () => {
  it("renders proper number of cells", () => {
    ...
    wrapper.setProps({
      ...
      config: {
        ...defaultColumns(5),
        options: { selectable: { selectBy: "test" } },
      },
    });
    ...
    const cellRenderer = (data, row, index, rowIndex) => (
      <div className="test-class-wrapper">
        <div className="test-class">
          {data} {index} {rowIndex} {row.test}
        </div>
      </div>
    );
    const props = getDefaultTestProps({
      config: { ...defaultColumns(1, { cellRenderer }) },
    });
