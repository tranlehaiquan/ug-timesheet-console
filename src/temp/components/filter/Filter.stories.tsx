...
storiesOf("Filter", module)
  .add("Filter", () => {
    ...
    return <Filter filters={filtersData} onFilter={onFilter} />;
  })
  .add("Filter example", () => {
    const filtersData: any = [
      {
  ...
  .add("Dropdown list", () => {
    ...
    const list = onlyList
      ? options.reduce(
          (opts, _, index) => [...opts, `${optionPrefix} ${index + 1}`],
          []
        )
      : options.reduce(
          (opts, _, index) => ({
            ...opts,
            [`${optionPrefix} ${index + 1}`]: false,
          }),
          {}
        );
    ...
    return (
      <DropdownList
        ...
        onOptionClick={
          boolean("onOptionClick", true) ? (label) => () => {} : null
        }
