...
export const BaseModal: React.SFC = ({ children, ...otherProps }) => {
  return (
    <Portal>
      ...
      <div className="sk-fade-in sk-fixed sk-pin sked-modal-bg sk-flex sk-justify-center sk-items-center sk-font-sans" data-sk-name="sked-modal">
        <div { ...otherProps } className="sk-bg-white sk-text-sm sk-rounded-medium sk-absolute sked-modal-position-top sk-shadow-lg">{ children }</div>
      </div>
    </Portal>
  )
}
...
export class ConfirmationModal extends React.PureComponent<IConfirmationModalProps, IConfirmationModalState> {
  ...
  render() {
    const { children, onCancel, onConfirm, confirmButtonText, ...rest } = this.props
      ...
      <BaseModal { ...otherProps }>
        <div className="sk-pt-10 sk-px-6 sk-pb-6 sk-flex sk-flex-col sked-modal-small">
          ...
          <div className="sk-text-right">
            <ButtonGroup>
              <Button data-sk-name="cancel-button" buttonType="transparent" onClick={ onCancel }>Cancel</Button>
              <Button data-sk-name="confirm-button" buttonType="primary" onClick={ this.handleConfirmButtonClick } loading={ this.state.confirmActionLoading }>{ confirmButtonText || 'Yes' }</Button>
            </ButtonGroup>
          </div>
        </div>
      </BaseModal>
