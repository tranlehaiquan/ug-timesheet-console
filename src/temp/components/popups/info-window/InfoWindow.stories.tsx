...
storiesOf("InfoWindow", module)
  ...
  .add(
    "Positioning",
    () => {
      const { position, triggerStyles, buttonPosition } =
        generatePosistioningKnobs();
      const { event } = generateTriggerKnobs();
      const { buttonText, content } = generateContentKnobs();

      return (
        <InfoWindow
          key={event + position + buttonPosition}
          event={event}
          position={position}
          content={content}
          style={triggerStyles}
...
const generateContentKnobs = () => {
  ...
  const content = (
    <div style={{ padding: "16px", maxWidth: "200px" }}>{contentText}</div>
  );
