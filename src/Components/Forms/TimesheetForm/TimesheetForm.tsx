...
export class TimesheetForm extends React.PureComponent<Props, TimesheetFormState> {
  ...
  componentDidUpdate(prevProps: Props, { StartDate: prevStart, ResourceId: prevId }: Partial<TimesheetFormState>) {
    const { StartDate, ResourceId, validationError } = this.state
    if (!!validationError && (StartDate !== prevStart || prevId !== ResourceId)) {
      this.setState({
  ...
  simpleChange = (fieldName: 'ResourceId', valuePath: string) => (dataObject: object) => {
    this.setState({
      ...
      if (valid) {
        ...
        this.setState({
          validationError: `A Timesheet has already been created within this date range for ${
            this.props.resources.find(resource => resource.UID === this.state.ResourceId).Name
          }`
        })
      }
    } catch (error) {
  ...
  validate = async ({ StartDate, EndDate, ResourceId }: { StartDate: string, EndDate: string, ResourceId: string}) => {
  ...
  dateChange: (key: 'StartDate' | 'EndDate') => ((date: Date) => void) = key => date => {
  ...
  render() {
    const resource = this.props.resources.find(({ UID }) => UID === this.state.ResourceId)
