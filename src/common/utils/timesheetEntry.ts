...
export const getNameOrDisplayName = (entry: ReduxDataTypes.TimesheetEntry, suffix: string) => {
...
export const getJobAllocationByTimeSheetEntry = (tsEntry: ReduxDataTypes.TimesheetEntry): ReduxDataTypes.JobAllocation => {
  if (tsEntry.EntryType === 'Job') {
    const { Timesheet: { ResourceId }, Job: { JobAllocations } } = tsEntry
    const jobAllocation = JobAllocations.find(ja => ja.ResourceId === ResourceId)
    return jobAllocation
  }
  return null
}

export const getActualLoggedTimeInMinute = (entry: ReduxDataTypes.TimesheetEntry): number => {
  const duration = entry.ActualDuration || calculateDurationInMinutes(
