...
storiesOf("Icons", module)
    ...
    <div className="sk-p-4">
      ...
      <div className="sk-flex sk-flex-wrap sk-items-center">
        {iconNames.map((icon, index) => {
          return (
            <div
              className="sk-w-32 sk-h-32 sk-m-1 sk-p-4 sk-border-3 sk-border-grey-lighter sk-flex sk-flex-col sk-justify-center sk-items-center sk-text-grey"
              key={`${name}-${index}`}
            >
  ...
  .add("Icon", () => (
    <Icon
      name={select("Icon name", iconNames, iconNames[0])}
      size={number("Size", 18)}
      onClick={action()}
    />
  ));
