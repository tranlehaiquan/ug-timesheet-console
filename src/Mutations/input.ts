...
import {
  makeActionsSet,
  makeReducers,
  makeAsyncActionCreatorSimp } from '../common/utils/redux-helpers'
import { ReduxDataTypes, UID } from '../Store/DataTypes'

interface CreationResp { schema: { insertTimesheet: UID } }
interface UpdateResp { schema: { updateTimesheet: UID }}
interface UpdateTimesheetEntryResp { schema: { updateTimesheetEntry: UID }}
interface DeleteResp { schema: { deleteTimesheet: UID }}
interface DeleteTimesheetEntryResp { schema: { deleteTimesheetEntry: UID } }
...
export const createTimesheet = makeAsyncActionCreatorSimp(
  TIMESHEET_CREATE, (createInput: ReduxDataTypes.Timesheet) => async (dispatch: Dispatch, getState: () => ReduxDataTypes.State) => {
    const { vendor } = getState()
    const { schema: { insertTimesheet } } = await Services.graphQL.mutate<CreationResp>({
...
export const updateTimesheetEntry = makeAsyncActionCreatorSimp(
  ENTRY_UPDATE, (updateInput: ReduxDataTypes.TimesheetEntry) => async (dispatch: Dispatch, getState: () => ReduxDataTypes.State) => {
    const { vendor } = getState()
    const { schema: { updateTimesheetEntry: updatedEntry } } = await Services.graphQL.mutate<UpdateTimesheetEntryResp>({
      query: updateTimesheetEntryQuery,
      variables: {
        updateInput: vendor === ReduxDataTypes.VendorType.SKEDULO ? updateInput : mergeDateFields(updateInput)
      }
    })
...
export const createTimesheetEntry = makeAsyncActionCreatorSimp(
  ENTRY_CREATE, (entries: ReduxDataTypes.TimesheetEntry[]) => async (dispatch: Dispatch, getState: () => ReduxDataTypes.State) => {
    const { vendor } = getState()
    const variables = entries.reduce((acc, entry, index) => ({
      ...acc,
      [`input${index}`]: vendor === ReduxDataTypes.VendorType.SKEDULO ? entry : mergeDateFields(entry)
    }), {})
    ...
    if (vendor === ReduxDataTypes.VendorType.SALESFORCE) {
      Object.values(result).forEach(entryResult => dispatch(getEntryById(Object.values(entryResult).pop())))
    }

    const { schema: { insertTimesheet } } = result
    return insertTimesheet
  }
)
...
export const updateTimesheetsStatus = makeAsyncActionCreatorSimp(
  TIMESHEET_STATUS_UPDATE, (
    ...
    const { vendor } = getState()
    const variables = timesheetsUIDs.reduce((acc, uid, index) => ({
      ...acc,
      [`input${index}`]: getUpdateTimesheetStatusVariables(uid, status, comment, userId)
    }), {})

    const { schema: { updateTimesheet } } = await Services.graphQL.mutate<UpdateResp>({
      variables,
...
export const deleteTimesheet = makeAsyncActionCreatorSimp(
  TIMESHEET_DELETE, (uid: UID) => async () => {
    const { schema: { deleteTimesheet: deletedUID } } = await Services.graphQL.mutate<DeleteResp>({
...
export const deleteTimesheetEntry = makeAsyncActionCreatorSimp(
  ENTRY_DELETE, (uid: UID) => async (dispatch: Dispatch, getState: () => ReduxDataTypes.State) => {
    const { vendor } = getState()
    const { schema: { deleteTimesheetEntry: deletedEntryUID } } = await Services.graphQL.mutate<DeleteTimesheetEntryResp>({
