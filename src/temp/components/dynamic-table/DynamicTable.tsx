...
export class DynamicTable<T> extends React.PureComponent<Props<T>, State> {
  constructor(props: Props<T>) {
    ...
    this.state = {
      sortDirection: get(
        this.props,
        "config.options.sortable.initialSortDirection",
        "asc"
      ) as sortDirection,
      sortedBy: get(this.props, "config.options.sortable.sortKey") as string,
      fixedColumnEnabled:
        (this.props.config.options &&
          this.props.config.options.fixedFirstColumn) ||
        false,
    };
  }
  ...
  componentDidMount() {
    !!this.props.config.options &&
      !!this.props.config.options.selectable &&
  ...
  checkColumnFixed = () => {
    ...
    if (this.tableContainerRef.current) {
      this.setState({
        fixedColumn: !(
          this.tableContainerRef.current.getBoundingClientRect().width >
          this.getInitialColsWidth()
        ),
      });
    }
  };
  ...
  getInitialColsWidth = () =>
    this.props.config.columns.reduce(
      (acc, curr) => (curr.width ? acc + curr.width : acc + 140),
      0
    );
  ...
  render() {
    ...
    return (
      <div
        ...
        data-sk-name={`sked-dynamic-table${
          this.state.fixedColumnEnabled ? "-fixed-first-col" : ""
        }${config.options && config.options.selectable ? "-selectable" : ""}`}
