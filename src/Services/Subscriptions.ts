...
export class Subscriptions {
  ...
  constructor(apiServer: string, token: string, connect = false) {
    ...
    this.ENDPOINT = server
      ? `wss://${server}/graphql/subscriptions`
      : GRAPHQL_WS_ENDPOINT;
    if (connect) {
      try {
        this.initConnection();
      ...
      this.wsClient = new SubscriptionClient(this.ENDPOINT, {
        ...
        connectionCallback: (err) => {
          if (err) {
            ...
            reject({
              connectionStatus: "Subscription connection error",
              error: err,
            });
          } else {
  ...
  register({
    ...
    this.apolloClient!.subscribe({
      ...
      next({ data }) {
        if (fetchMore) {
          (dispatch as ThunkDispatch<ReduxDataTypes.State, undefined, Action>)(
            fetchMoreHandler[subscriptionName](data)
          );
        } else {
          (dispatch as ThunkDispatch<ReduxDataTypes.State, undefined, Action>)(
            handleUpdateMessage({ subscriptionName, data })
          );
        }
      },
  ...
  // tslint:disable-next-line: no-console
  registerErrorHandler = (name: string) => (err: Error) =>
