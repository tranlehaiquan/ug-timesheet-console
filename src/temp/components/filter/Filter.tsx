...
export class Filter<T> extends React.PureComponent<FilterProps<T>, State<T>> {
  ...
  preparePreSelected = () => {
    ...
    const preselectedFilters = filters.filter(
    ...
    return preselectedFilters.map((filter) => ({
      ...
      options: filter.options.reduce(
        (acc, option) => ({
          ...acc,
          [option]: filter.preselected.includes(option),
        }),
        {}
      ),
    }));
    ...
    this.setState(
      {
        activeFilters: this.state.activeFilters.filter(
          ({ name, options }) =>
            name !== this.state.activeDropdown ||
            values(options).some((selected) => selected)
        ),
        activeDropdown: "",
      },
      cb
  ...
  addNewFilter = (name: string) => () => {
    ...
    const { filters } = this.props;
    const theFilter = filters.find(
      ({ name: filterName }) => name === filterName
    );
    const newFilterOptions = theFilter.options.reduce(
  ...
  newFilterOptions = () => {
    const selectedCount = this.state.activeFilters.reduce(
      (acc, { name, options }) => ({
        ...acc,
        [name]: Object.keys(pickBy(options)).length,
      }),
      {}
    );
    return (
    ...
    return (
      <div
        ...
        className={classnames(
          "sk-flex sk-w-full sk-flex-wrap sk-p-1",
          classNames
        )}
      >
