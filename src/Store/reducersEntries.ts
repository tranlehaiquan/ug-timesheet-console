...
import { calculateStats, reduceTimesheetEntriesData, formatEntryData } from './reducersTimesheets'
...
export const getEntryById = makeAsyncActionCreatorSimp(
  ENTRY_GET, (UID: UID) => async (dispatch: Dispatch, getState: () => ReduxDataTypes.State) => {
...
const entryGetTransform = (data: EntryGetResponse, store: ReduxDataTypes.State) => {
  ...
  if (timesheetData) {
    const entryTimesheet = timesheetData.find(({ UID }) => UID === newEntry.TimesheetId)
    if (entryTimesheet) {
      const alreadyStoredIndex = entryTimesheet.Entries.findIndex(entry => entry.UID === newEntry.UID)
      const newEntries = alreadyStoredIndex === -1
      ...
      newEntries.sort((entry1, entry2) => (`${entry1.StartDate} ${entry1.StartTime}` < `${entry2.StartDate} ${entry2.StartTime}` ? -1 : 1))
...
export const clearEntry = makeActionCreator(ENTRY_CLEAR, null, ['entryId', 'timesheetId'])
...
const entryClearReducer = (
  ...
  if (timesheetData) {
    const entryTimesheet = timesheetId
      ...
      : timesheetData.find(timesheet => !!timesheet.Entries.find(entry => entry.UID === entryId))
    if (entryTimesheet) {
      const newTimesheet = reduceTimesheetEntriesData({
        ...entryTimesheet,
        Entries: entryTimesheet.Entries.filter(entry => entry.UID !== entryId)
      })
