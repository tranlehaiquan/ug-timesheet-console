...
const wsSubscriptions = new Subscriptions(credentials.apiServer, auth.skedApiAccessToken)
...
export const registerNewSubscription = makeAsyncActionCreatorSimp(
  SUBSCRIPTION_REGISTER, (subscriptionName: string, fetch: boolean, variables?: {}) => (dispatch: Dispatch) => {
...
interface TimesheetEntryChangePayload {
  schemaTimesheetEntry: {
    ...
    data: Omit<
      ...
      'Activity' | 'DistanceUnit' | 'Duration' | 'Job' | 'Shift' | 'Timesheet' | 'Unavailability'
    >
  }
}
...
export const fetchMoreHandler:
  {
    ...
    TimesheetUpdate: (data: TimesheetChangePayload) => (dispatch: Dispatch) => {
      const { schemaTimesheet: { operation } } = data
    ...
    TimesheetEntryChange: (data: TimesheetEntryChangePayload) => (dispatch: Dispatch) => {
      const { schemaTimesheetEntry: { operation, data: { UID, TimesheetId } } } = data
...
const updateHandlers: {[key: string]: (data: TimesheetChangePayload, state: ReduxDataTypes.State) => void } = {
  TimesheetUpdate: (data: TimesheetChangePayload, state: ReduxDataTypes.State) => {
    const {
      schemaTimesheet: {
        operation,
        data: {
    ...
    if (timesheetData && timesheetData.length) {
      const timesheet = timesheetData.find(({ UID: savedUID }) => UID === savedUID)
      if (timesheet) {
        const newData = {
          ...timesheet,
...
export const subscriptionReducers = {
  ...makeReducers(SUBSCRIPTION_INIT, { transform: ({ connectionStatus }: { connectionStatus: string }) => {
    return ({ subscriptionStatus: connectionStatus })
  } }),
  [SUBSCRIPTION_UPDATE]: (state: ReduxDataTypes.State, message: { subscriptionName: string, data: TimesheetChangePayload }) => {
