...
import { ReduxDataTypes, EntryTypes, DistanceUnit, UID } from '../../../Store/DataTypes'
import { createTimesheetEntry, updateTimesheetEntry } from '../../../Mutations/input'
...
import { parseLocalDateToZonedDate, parseZonedDateToLocalDate } from '../Utils/date'
import delay from '../../../common/utils/delayPromise'

const ENTRY_TYPES: EntryTypes[] = ['Job', 'Shift', 'Activity', 'Unavailability', 'Manual']
...
const getEntryType: (timesheetEntry: ReduxDataTypes.TimesheetEntry) => EntryTypes = timesheetEntry => {
...
const timesheetEntryToState = (
  ...
  const Start = StartDate && StartTime ? moment.utc(`${StartDate} ${StartTime}`, 'YYYY-MM-DD HH:mm').toISOString() : undefined
  const End = EndDate && EndTime ? moment.utc(`${EndDate} ${EndTime}`, 'YYYY-MM-DD HH:mm').toISOString() : undefined
  ...
  return {
    ...
    Description,
    Distance: Distance
      ? distance! % 1 ? distance!.toFixed(2).toString() : distance.toString()
      : undefined,
    DistanceUnit: 'KM' as DistanceUnit,
    EntryType: entryType,
    Name: entryType === 'Manual' ? entryType : undefined,
    Duration: calculateDurationInMinutes(Start, End),
    selectedBreaksIds: entryType === 'Shift' ? (new Set() as Set<string>) : undefined
  }
}
...
const convertPremiumDataToSelectOptions = (
  ...
  return options.filter(option => premiums.findIndex(premium => premium === option.value) > -1)
}
...
export class TimesheetEntryForm extends React.PureComponent<Props, State> {
  constructor(props: Props) {
    ...
    // TODO: option mby?
    const timesheetEntry = this.props.timesheetData!
      .find(({ UID: tsUID }) => tsUID === props.TimesheetUID)!
      .Entries
      .find(({ UID: entryUID }) => entryUID === props.TimesheetEntryUID) || undefined
    ...
    this.state = {
      ...RESET_STATE,
      ResourceUID: props.timesheetData!.find(({ UID }) => UID === props.TimesheetUID)!.Resource.UID,
      ...
      timezone: timesheetEntry && timesheetEntry.Timezone ? timesheetEntry.Timezone : props.settings.defaultTimezone,
      LunchBreakFormValue: timesheetEntry && timesheetEntry.LunchBreakDuration || 0,
      PremiumOptions: [],
      PremiumsFormValue: timesheetEntry && timesheetEntry.Premiums && (JSON.parse(timesheetEntry.Premiums) as string[]).map(premium => ({ label: premium, value: premium })) || [],
      loading: false
    }
  }
  ...
  async componentWillMount() {
    ...
    if (entryType === 'Job') {
      ...
      const resource = this.props.timesheetData!.find(({ UID }) => UID === this.props.TimesheetUID)!.Resource
      premiumOptions = result
        .filter(item => {
          return item.active && item.validFor && (item.validFor.length === 0 || item.validFor.includes(resource.Category))
        })
        .map(rawOption => ({
  ...
  componentDidUpdate(_prevProps: Props, prevState: State) {
    if (!!this.state.validationError && this.validatedFieldChanged(prevState, this.state)) {
      this.setState({
  ...
  getNameOfRelatedActivity() {
    ...
    const newEntries = this.props[`newEntry${entryType}` as keyof ReduxNewEntries]
    if (!newEntries) {
      return undefined
    }

    const relatedEntryId = this.state[`${entryType}Id` as keyof StateEntries]
    const relatedEntry = newEntries.find(({ UID: entryUID }) => entryUID === relatedEntryId!)

    return relatedEntry ? relatedEntry.Name || (relatedEntry as ReduxDataTypes.Shift).DisplayName || relatedEntry.UID : undefined
  }
  ...
  handleSimpleChange = (key: 'Description' | 'DistanceUnit', path = 'target.value') => (e: React.SyntheticEvent) => {
    this.setState({
      ...this.state,
      [key]: get(e, path, '')
    })
  ...
  getPrefilledDescription = (entryType: EntryTypes, data: ReduxDataTypes.TimesheetEntryRelation) => {
    let description
    switch (entryType) {
      case 'Job':
        ...
        break
      case 'Shift':
        description = (data as ReduxDataTypes.Shift).Location ? `${(data as ReduxDataTypes.Shift).Location.Name}` : null
        break
      case 'Activity':
      case 'Unavailability':
        description = (data as ReduxDataTypes.Activity | ReduxDataTypes.Availability).Type
        break
    }
  ...
  handleDateChange = (type: 'Start' | 'End') => (value: Date) => {
    const valueISO = value ? parseZonedDateToLocalDate(value, this.state.timezone).toISOString() : null
  ...
  handleDistanceChange = ({ target: { value } }: { target: { value: string } }) => {
  ...
  limitEntryToTimesheetPeriod = (item: State) => {
    ...
    const { StartDate, EndDate } = timesheetData!.find(({ UID }) => UID === TimesheetUID)!
    ...
    if (itemStartDate.isBefore(timesheetStartDate, 'days') || itemStartDate.isAfter(timesheetEndDate, 'days')) {
    ...
    if (itemEndDate.isAfter(timesheetEndDate, 'days') || itemEndDate.isBefore(timesheetStartDate, 'days')) {
  ...
  handleNameChange = (data: ReduxDataTypes.TimesheetEntryRelation) => () => {
    ...
    const { LunchBreakDuration, Premiums, JobAllocationId } = data as ReduxDataTypes.Job
    const endDate = (data as ReduxDataTypes.Job).End || (data as ReduxDataTypes.Availability).Finish
    ...
    const newState = this.limitEntryToTimesheetPeriod({
      ...
      Start,
      DisplayName,
      Breaks: Breaks && Breaks.slice().sort((Break1: ReduxDataTypes.Break, Break2: ReduxDataTypes.Break) => (Break1.Start < Break2.Start ? -1 : 1)),
      [`${EntryType}Id`]: UID,
      End: endDate,
      Name: Name || (data as ReduxDataTypes.Shift).DisplayName || '-',
      Distance:
        EntryType === 'Job' && (data as ReduxDataTypes.Job).TravelDistance
          ? convertDistance((data as ReduxDataTypes.Job).TravelDistance, this.state.DistanceUnit).toFixed(2).toString()
          : undefined,
      selectedBreaksIds:
        EntryType === 'Shift'
      ...
      PremiumsFormValue: convertPremiumDataToSelectOptions(PremiumOptions, Premiums)
    })

    if (JobAllocationId) {
      newState.JobAllocationId = JobAllocationId
    }
    newState.Duration = calculateDurationInMinutes(newState.Start, newState.End)
  ...
  handleTypeChange = (value: EntryTypes) => () => {
    this.setState({
      ...
      Start: value === 'Manual' ? this.getMinMaxDates().minDate.toISOString() : undefined,
      End: value === 'Manual' ? this.getMinMaxDates().minDate.toISOString() : undefined,
      PremiumsFormValue: [],
      LunchBreakFormValue: 0
    })

    if (value !== 'Manual' && (this.props[`newEntry${value}` as keyof ReduxProps] === undefined)) {
  ...
  setValidationError = (invalidFields: ValidationField[], errorMessage: string) => {
    this.setState({
      invalidFields,
  ...
  isValid = () => {
    ...
    if (entryDuration < 0) {
      this.setState({ validationError: 'End time cannot be before start time' })
      return false
    } if (entryDuration === 0) {
      this.setState({ validationError: 'End time cannot be the same as start time' })
      return false
    }
  ...
  toDistanceFormat = (
    ...
    if ((entryType !== 'Job' && entryType !== 'Manual') || !numericDistance) return null
    return distanceUnit === 'KM'
  ...
  getNewEntryInput(
    ...
    return {
      ...
      Duration: calculateDurationInMinutes(`${StartDate}T${StartTime}Z`, `${EndDate}T${EndTime}Z`) - (LunchBreakFormValue || 0),
      LunchBreakDuration: LunchBreakFormValue || 0,
      Premiums: PremiumsFormValue && PremiumsFormValue.length ? JSON.stringify(PremiumsFormValue.map(premium => premium.value)) : null
    }
  }

  getNewShiftBreakEntryInput: (Break: ReduxDataTypes.Break) => Partial<ReduxDataTypes.TimesheetEntry> = Break => {
    const [StartDate, StartTime] = new Date(Break.Start!).toISOString().split('T')
  ...
  getSplitShiftEntryInput() {
    const selectedBreaks = this.state.Breaks!
      .filter(Break => this.state.selectedBreaksIds!.has(Break.UID))
    ...
    if (start !== this.state.End) {
      inputs.push(this.getNewEntryInput({ ...this.state, Start: start }, this.state.DistanceUnit))
    }
  ...
  mapComparedTSEntry = (entry: Partial<ReduxDataTypes.TimesheetEntry>): IComparedTSEntry => {
  ...
  checkDuplicatedTSEntry = (timesheet: ReduxDataTypes.TimesheetData, entryToCompare: Partial<ReduxDataTypes.TimesheetEntry>): boolean => {
    const entry = timesheet.Entries.find(entry => {
      const currInput = this.mapComparedTSEntry(entry)
      return JSON.stringify(currInput) === JSON.stringify(this.mapComparedTSEntry(entryToCompare))
    })
    return entry && !!entry.UID
  }

  validateDuplicateTimesheetEntry = (newInput: Partial<ReduxDataTypes.TimesheetEntry>): boolean => {
  ...
  handleSave = async () => {
    ...
    try {
      ...
      if (UID) {
        const { timesheetData, TimesheetUID } = this.props
        const { Status } = timesheetData!.find(({ UID }) => UID === TimesheetUID)!
        const isJobAndApprovedTS = this.state.EntryType === 'Job' && Status === 'Approved'
        const payload = {
          ...input,
          IsNew: !isJobAndApprovedTS
        }
        await this.props.updateTimesheetEntry(payload as ReduxDataTypes.TimesheetEntry)
        if (isJobAndApprovedTS) {
          const timesheetEntry = this.getTimesheetEntry((input as ReduxDataTypes.TimesheetEntry).UID)
          if (timesheetEntry.JobAllocationId) {
            // Because we use webhook to update JA
            // workaround to make sure generate latest JA
            await delay(5000)
            await dataService.triggerGeneratePdf(true, [timesheetEntry.JobAllocationId])
          }
        }
      } else {
  ...
  getMinMaxDates = () => {
    const { timesheetData, TimesheetUID } = this.props
    const selectedTimesheetEntry = timesheetData!.find(({ UID }) => UID === TimesheetUID)!

    if (!get(selectedTimesheetEntry, 'StartDate') || !get(selectedTimesheetEntry, 'EndDate')) {
      return {
        minDate: new Date(),
        maxDate: new Date()
      }
  ...
  getRelatedToName = () => {
    if (this.state.EntryType === 'Shift' && this.state.ShiftId) {
      const name = (this.state.Name && this.state.Name !== '-' && this.state.Name !== this.state.ShiftId) ? this.state.Name : this.state.DisplayName
      return name || `Shift ${format(new Date(this.state.Start), 'h:mm a')} - ${format(new Date(this.state.End), 'h:mm a')}`
    }
    if (this.state.EntryType === 'Unavailability' && this.props.newEntryUnavailability && this.state.UnavailabilityId) {
      return (this.props.newEntryUnavailability.find(({ UID }) => UID === this.state.UnavailabilityId) as ReduxDataTypes.Availability).Type
    }
    ...
    return (
      <>
        <div className="sk-mb-8">
          ...
          <DatePicker
            selected={ this.state.Start ? parseLocalDateToZonedDate(this.state.Start, this.state.timezone) : null }
        ...
        <div className="sk-mb-8">
          <Label text="End Date" />
          <DatePicker
            selected={ this.state.End ? parseLocalDateToZonedDate(this.state.End, this.state.timezone) : null }
        ...
        <div className="sk-mb-8 sk-flex">
          <div className="sk-w-1/2 sk-mr-3">
            ...
            <TimePicker
              selected={ (this.state.Start ? parseLocalDateToZonedDate(this.state.Start, this.state.timezone) : undefined) }
          ...
          <div className="sk-w-1/2 sk-ml-3">
            <Label text="End Time" />
            <TimePicker
              selected={ this.state.End ? parseLocalDateToZonedDate(this.state.End, this.state.timezone) : undefined }
        ...
        <div className="sk-mb-8 sk-flex">
          <div className="sk-w-1/2 sk-mr-3">
            <Label text="Duration" />
            <FormInputElement disabled value={ minutesToDuration(this.state.Duration) || '' } />
          </div>
          { (this.state.EntryType === 'Job' || this.state.EntryType === 'Manual') && (
        ...
        { this.state.EntryType === 'Job' && (
          <>
            <div className="sk-mb-8">
              ...
              <Select
                label={ LUNCH_BREAK_OPTIONS.find(option => option.value === this.state.LunchBreakFormValue).label }
                optionLabel="label"
                disabled={ this.state.EntryType !== 'Job' }
                data={ LUNCH_BREAK_OPTIONS }
                onOptionClick={ (data: object) => { this.handleLunchBreakChange(data as {label: string, value: number}) } }
              />
            </div>
            <div className="sk-mb-8">
              <Label text="Premiums" />
    ...
    return (
      <>
        <div className="sk-mb-8">
          <Label text="Start Date" />
          <DatePicker
            selected={ this.state.Start ? moment.utc(this.state.Start).tz(this.state.timezone).toDate() : null }
            onChange={ this.handleDateChange('Start') }
            { ...this.getMinMaxDates() }
          />
        </div>
        ...
        <div className="sk-mb-8 sk-flex">
          <div className="sk-w-1/3 sk-mr-3">
            <Label text="Start Time" />
            <TimePicker
              selected={ (this.state.Start ? moment(this.state.Start).toDate() : undefined) }
              onChange={ this.handleTimeChange('Start') }
              disabled={ !this.state.Start }
            />
          </div>
        ...
        { this.renderDescriptionField() }
        {
          ...
          <BreaksSelect
            ...
            onChange={ selections => this.setState({ selectedBreaksIds: selections }) }
  ...
  render() {
    const { EntryType, visibleDropdown, validationError, isFinished } = this.state
    ...
    return (
      ...
      >
        ...
        }
        {
          ...
          <ValidationWarning>
            You are adding this shift before the resource has started or completed the shift
             - please note that shift breaks will not be deducted from the duration of this timesheet entry if you continue
          </ValidationWarning>
        }
