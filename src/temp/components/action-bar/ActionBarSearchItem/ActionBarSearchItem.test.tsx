...
describe('ActionBarSearchItem', () => {
  ...
  const clearInputButton = 'button[data-sk-name="action-bar-search-clear-button"]'
  ...
  it('updates input value when typing', () => {
    ...
    wrapper.find(searchInput).simulate('change', { target: { value: 'another test' } })
    expect(wrapper.find(clearInputButton).length).toEqual(1)
    expect(wrapper.find(searchInput).get(0).props.value).toEqual('another test')

    wrapper.find(searchInput).simulate('change', { target: { value: 'yet another test' } })
    expect(wrapper.find(clearInputButton).length).toEqual(1)
    expect(wrapper.find(searchInput).get(0).props.value).toEqual('yet another test')
  })
})
