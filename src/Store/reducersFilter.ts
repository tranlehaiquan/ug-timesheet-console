...
const customFiltersOptionsMap: {
  [index: string]: (name: string, resources: ReduxDataTypes.Resource[]) => string[]
} = {
...
const createFilter = (name: keyof ReduxDataTypes.Resource, resources: ReduxDataTypes.Resource[]) => {
  ...
  return { name, options, selector: filtesrMap[name].selector, value: (null as string[]), description: filtesrMap[name].label }
}

function createSimpleTypeFilterOptions(name: keyof ReduxDataTypes.Resource, resources: ReduxDataTypes.Resource[]) {
  const allOptions = new Set(resources
...
function createTagsFilterOptions(name: string, resources: ReduxDataTypes.Resource[]) {
  const allTags = resources.reduce((acc: string[], resource) => {
    const tags = resource.ResourceTags.map(resourceTags => resourceTags.Tag.Name)
    return [...acc, ...tags]
  }, [])
...
function createRegionsFilterOptions(name: string, resources: ReduxDataTypes.Resource[]) {
  const allRegions = resources.reduce((acc: string[], resource) => {
    const regions = resource.ResourceRegions.map(resourceTags => resourceTags.Region.Name)
    return [...acc, ...regions]
  }, [])
...
function createPrimaryRegionFilterOptions(name: string, resources: ReduxDataTypes.Resource[]) {
...
const buildFilters = (resources: ReduxDataTypes.Resource[]) => {
  const filterNames = Object.keys(resources[0] || {}) as (keyof ReduxDataTypes.Resource)[]
...
export const setupFilters = makeAsyncActionCreatorSimp(
  FILTERS_SETUP, () => (dispatch: Dispatch) => {
...
export const setFiltersValues = makeAsyncActionCreatorSimp(
  TIMESHEETS_FILTER, (values: { name: string, value: string[] }[]) => (dispatch: Dispatch) => {
