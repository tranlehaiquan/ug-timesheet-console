...
import { ResponsiveDropdown, PopOutBase, Button, Icon, FormInputElement, LoadingSpinner, DropdownListProps } from 'skedulo-ui'
...
const selectData = (type: EntryTypes, searchValue: string) => (state: ReduxDataTypes.State) => {
  const data: (ReduxDataTypes.Job | ReduxDataTypes.Shift | ReduxDataTypes.Activity | ReduxDataTypes.Availability)[] = state[`newEntry${type}`]
  ...
  return searchValue
    ? data.filter(item => searchObject(item, ['Name', 'DisplayName'], searchValue))
    : data
}
...
const ListLoadingIndicator: React.FC = ({ children }) => {
  const busyState = useSelector(({ busy, busyCnt }: ReduxDataTypes.State) => busy && busyCnt.includes('NEW_ENTRY'))
  return (
    <>
      { busyState
...
const ArrayOptionRenderer: React.FC<{
  ...
  onOptionClick: (option: ReduxDataTypes.TimesheetEntryRelation) => (ev: React.MouseEvent) => void
  ...
  return (
    <div data-sk-name="sk-dropdownlist" className="sk-dropdownlist" style={ { maxWidth: 'auto' } }>
      { children }
      { type !== 'Manual' && (
        <>
          <SearchItem onSearch={ onSearchHandler } searchPlaceholder={ searchPlaceholder || 'Search' } />
          <DropdownBr />
            ...
            <ListLoadingIndicator>
              { options && options.length ? options.map((option, index) => (
                <DropdownRow
                  ...
                  key={ index }
                >
                  <OptionRenderer type={ type } option={ option } timezone={ timezone } />
                </DropdownRow>
...
const OptionRenderer: React.FC<{ type: EntryTypes, option: ReduxDataTypes.TimesheetEntryRelation, timezone: string }> = ({ option, type, timezone }) => {
  return (
    <div className="sk-flex sk-items-stretch sk-content-center sk-mb-1 sk-mt-1">
      ...
      <div>
        ...
        <div className="sk-text-xxs sk-text-grey sk-whitespace-pre-wrap">{ summaryDisplay(type, option, timezone) }</div>
      </div>
    </div>
  )
}

const getFormattedTime = (option: ReduxDataTypes.TimesheetEntryRelation, timezone: string) => {
  ...
  const end = (
    (option as ReduxDataTypes.Shift | ReduxDataTypes.Job | ReduxDataTypes.Activity).End ||
    (option as ReduxDataTypes.Availability).Finish
  )
...
const getOptionName = (option: ReduxDataTypes.TimesheetEntryRelation, type: EntryTypes, timezone: string) => {
  if (type === 'Shift') {
    const { startDate, endDate, startTime, endTime } = getFormattedTime(option, timezone)
    const shiftOption = option as ReduxDataTypes.Shift
    return shiftOption.DisplayName ? shiftOption.DisplayName : `Shift ${startDate} ${startTime} - ${endDate} ${endTime}`
  }
...
const summaryDisplay = (type: EntryTypes, option: ReduxDataTypes.TimesheetEntryRelation, timezone: string) => {
  const { startDate, endDate, startTime, endTime } = getFormattedTime(option, timezone)

  if (type === 'Job') {
    const o = option as ReduxDataTypes.Job
    return `${o.Type}${o.Account && o.Account.Name ? ` | ${o.Account.Name}` : ''} | ${o.JobStatus}\n${startDate} ${startTime} - ${endDate} ${endTime}`
  }
  if (type === 'Shift') {
    const o = option as ReduxDataTypes.Shift
    return o.DisplayName ? `${startDate} ${startTime} - ${endDate} ${endTime}` : ''
  }
  if (type === 'Unavailability') {
    return `${startDate} ${startTime} - ${endDate} ${endTime}`
  }
  if (type === 'Activity') {
    return `${(option as ReduxDataTypes.Activity).Type}\n${startDate} ${startTime} - ${endDate} ${endTime}`
  }

  return `${type} (${startDate} ${startTime} - ${endDate} ${endTime})`
}

const TypeSelect: React.FC<{ data: EntryTypes[], onClick: (value: EntryTypes) => () => void, selectedType: string }> = ({ data, onClick, selectedType }) => {
  return (
    <div className="tse-tabs">
      { data.map(element => (
        <div
          key={ element }
          className={ classnames('tse-tabs__element', { 'tse-tabs__element--selected': element === selectedType }) }
