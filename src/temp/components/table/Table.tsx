  ...
  return (
    <table
      className={classnames(
        "sk-w-full sk-border-collapse sk-table-fixed",
        className
      )}
      {...otherProps}
    >
...
export const TableHead: React.FC<ITableHeadProps> = (props) => {
  return (
    <thead
      className={classnames(
        "sk-bt-2 sk-bb-2 sk-border-t sk-border-b sk-border-grey-light sk-bg-grey-lightest sk-text-navy-lighter sk-text-xxs sk-uppercase",
        props.className
      )}
    >
...
export const TableHeaderCell: React.FC<ITableHeaderCellProps> = ({
  ...
  const [widthToUse, setWidth] = React.useState<string | number>(
    cell.width || ""
  );
  const min = cell.minWidth ? cell.minWidth : 120;

  const rend = <Condenser>{cell.name}</Condenser>;
  ...
  return (
    <th
      ...
      style={{
        width: widthToUse || `${cell.width}px`,
        minWidth: `${min}px`,
        position: "sticky",
      }}
    >
...
> = (props) => {
  ...
  return (
    <tr
      className={classnames("sk-border-b sk-border-grey-lighter", className)}
      {...otherProps}
    >
      {children}
    </tr>
...
> = (props) => {
  ...
  return (
    <td
      className={classnames("sk-py-3 sk-px-5 sk-text-xs", className)}
      {...otherProps}
    >
      {children}
    </td>
