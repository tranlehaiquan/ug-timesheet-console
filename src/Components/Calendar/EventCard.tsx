...
export const EventCard: React.FC<Props> = ({
  ...
  const startsThisDay = isWithinInterval(event.start, {
    start: startOfDay(day),
    end: endOfDay(day),
  });
  const endsThisDay = isWithinInterval(event.end, {
    start: startOfDay(day),
    end: endOfDay(day),
  });
  ...
  return (
    <div
      ...
      style={{
        ...
        gridRowStart:
          isRecurrent || startsThisDay ? calculateRowStart(event) : 1,
        gridRowEnd: isRecurrent || endsThisDay ? calculateRowEnd(event) : 25,
        marginTop:
          isRecurrent || startsThisDay
            ? calculateTopMargin(event, cellHeight)
            : 0,
        marginBottom:
          isRecurrent || endsThisDay
            ? calculateBottomMargin(event, cellHeight)
            : 4,
