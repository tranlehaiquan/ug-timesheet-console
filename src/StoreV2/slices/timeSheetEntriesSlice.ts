...
import { FAILED_FETCH_TS_ENTRY, FAILED_UPDATE_TS_ENTRY_STATUS } from '../../common/constants/errorMessage'
...
const createAllTimeSheetsEntryFilters = (
  ...
  const dateFilter = (startDate: string, endDate: string) => `Timesheet.StartDate >= ${startDate} AND Timesheet.EndDate <= ${endDate}`
  ...
  const appliedFilters = filterValues
    ...
    .map(
      ({ filterValues, selector }) => `${selector} ${formatFilterValue(filterValues)}`
    )
...
>(`${SLICE_NAME}/fetchAll`, async (params, thunkAPI) => {
  ...
  const orderBy = state.order.orderBy ? `${state.order.orderBy.field} ${state.order.orderBy.direction.toUpperCase()}` : 'CreatedDate'
...
export const selectTimeSheetEntries = (state: RootState) => state.timeSheetEntries.values || []
