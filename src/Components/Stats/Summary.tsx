...
import { breakHoursToHoursAndMinutes, breakMilisecondsToHoursAndMinutes } from '../../common/utils/dateTimeHelpers'
import { metersToKilometers, metersToMiles } from '../../common/utils/distanceUnits'
  ...
  return (
    <>
      ...
      time.hours
        ? (
          <>
            <span className="summary__number">
              { time.hours }
            </span>
...
const renderDistance = (distance: number, distanceUnit: ReduxDataTypes.DistanceUnit) => {
  const convertedDistance = distanceUnit === 'MI' ? metersToMiles(distance) : metersToKilometers(distance)
...
export const Summary: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const distanceUnit = useSelector((state: RootState) => state.setting.distanceUnit)
  const unavailabilityTime = breakHoursToHoursAndMinutes(summary.unavailability)
  ...
  return (
    <div className={ classNames('summary', className) }>
      ...
      <span className="summary__number summary__value--1">
        { summary.jobs }
      </span>
      ...
      <span className="summary__value--2">
        { renderTimeValue(loggedTime) }
      </span>
