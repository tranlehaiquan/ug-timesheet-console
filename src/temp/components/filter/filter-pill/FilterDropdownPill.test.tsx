...
describe("Filter Dropdown Pill", () => {
  it("renders array list", () => {
    ...
    const onClickHandlers = OPTIONS.reduce(
      (acc, optionName) => ({
        ...acc,
        [optionName]: jest.fn(),
      }),
    ...
    wrapper.setProps({
      onOptionClick: (label) => onClickHandlers[label].mockReturnValue(label),
    });
    expect(wrapper.find(PopOutBase).prop("visible")).toEqual(false);
    wrapper.setProps({ isActive: true });
    expect(wrapper.find(PopOutBase).prop("visible")).toEqual(true);

    expect(wrapper.find(attrName("sk-filter-dropdown-row", "li"))).toHaveLength(
      OPTIONS.length
    );

    wrapper
      .find(attrName("sk-filter-dropdown-row", "li"))
      .at(2)
      .simulate("click");
    ...
    expect(
      wrapper.find(attrName("sk-filter-search-input", "input"))
    ).toHaveLength(0);
    ...
    expect(
      wrapper.find(attrName("sk-filter-search-input", "input")).is(":focus")
    ).toBe(true);

    wrapper
      .find(attrName("sk-filter-search-input", "input"))
      .simulate("change", { target: { value: "no match" } });
    expect(wrapper.find(attrName("sk-filter-dropdown-row", "li"))).toHaveLength(
      0
    );
    expect(wrapper.find(attrName("sk-filter-empty-state"))).toHaveLength(1);
    expect(wrapper.find(attrName("sk-filter-empty-state")).text()).toEqual(
      "No results found"
    );
    ...
    wrapper
      .find(attrName("sk-filter-search-input", "input"))
      .simulate("change", { target: { value: "Option" } });
    expect(wrapper.find(attrName("sk-filter-dropdown-row", "li"))).toHaveLength(
      3
    );
    ...
    expect(
      wrapper.find(attrName("sk-filter-dropdown-row", "li")).text()
    ).toEqual(OPTIONS[1]);
  });
  ...
  it("renders input list", () => {
    const wrapper = getWrapper({
      filter: {
        ...
        options: OPTIONS.reduce(
          (acc, optionName) => ({ ...acc, [optionName]: false }),
          {} as Options
        ),
      },
    });
    ...
    const applyButton = () =>
      wrapper.find(attrName("sk-filter-apply", "button"));
    const listElements = () =>
      wrapper.find(attrName("sk-filter-dropdown-row", "li"));
    ...
    wrapper.setProps({
      onApply: (options) => onApply.mockReturnValue(options),
    });
  ...
  it("renders search only list", () => {
    ...
    wrapper.setProps({
      onApply: (options) =>
        onApply.mockReturnValue(Object.keys(pickBy(options)).length),
    });
