...
export const getVendorInfo = makeAsyncActionCreatorSimp(
  ...
  () => async () => {
    const {
      vendorInfo: { vendor },
    } = await Services.metadata._apiService.get<VendorResp>("auth/whoami");
    return { vendor };
  }
);
...
export const getResources = makeAsyncActionCreatorSimp(
  ...
  () => async (dispatch: Dispatch) => {
    ...
    dispatch(
      getUsersAvatars(
        resources.reduce(
          (acc, { User }) => (User ? [...acc, User.UID] : acc),
          [] as UID[]
        )
      )
    );
  ...
  (ids: UID[]) => async () => {
    if (ids.length > 0) {
      ...
      const avatars = await Promise.all(
        chunkedIds.map((chunkedPack) =>
          Services.metadata._apiService.get(
...
export const getNewEntryData = makeAsyncActionCreatorSimp(
  NEW_ENTRY,
  (entryType: DefinedEntryTypes, resourceUID: UID) =>
    async (dispatch: Dispatch, getState: () => ReduxDataTypes.State) => {
      const store = getState();
      const resp = await dataService.fetchGraphQl<
        | ReduxDataTypes.Job
...
const newEntryShiftTransform = (resourceShifts: ResourceShift[]) =>
  resourceShifts.map((resourceShift: ResourceShift) => {
...
const newEntryTransform = (
  { jobAllocations, resourceShifts, activities, availabilities }: NewEntryResp,
        ...
        uniqBy(
          [
            ...jobAllocations.filter(
              (item) =>
                item.Job.JobStatus !== "Cancelled" && item.Status !== "Deleted"
            ),
            ...jobAllocations.filter(
