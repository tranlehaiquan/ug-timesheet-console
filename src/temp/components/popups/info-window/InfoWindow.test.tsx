...
describe("InfoWindow", () => {
  afterEach(() => {
    ...
    if (document.getElementsByClassName("sked-infowindow").length) {
      document.body.removeChild(
        document.getElementsByClassName("sked-infowindow")[0]
      );
    }
  });
  ...
  test("renders the content on click", () => {
    ...
    expect(document.getElementsByClassName("sked-infowindow").length).toEqual(
      1
    );
  });
  ...
  describe("delayShow", () => {
    ...
    beforeEach(() => {
      attachEventListenersSpy = jest.spyOn(
    ...
    it("renders content after a delay", (done: () => void) => {
      ...
      setTimeout(() => {
        expect(
          document.getElementsByClassName("sked-infowindow").length
        ).toEqual(1);
        done();
      }, 20);
    });
  ...
  describe("getDisplayPosition", () => {
    it("updates trigger rectangle based on window scroll", () => {
      // Arrange
      const getPositionScoresSpy = jest.spyOn(
        InfoWindowPositionUtilsModule,
        "getPositionScores"
      );
      // Fake a scroll
