...
const TodayButton: React.FC<ITodayButtonProps> = ({ onDateSelect }) => {
  ...
  return (
    <Button buttonType="secondary" onClick={ todayHandler }>Today</Button>
  )
}

const NavigationButtons: React.FC<INavigationButtonProps> = ({ onDateSelect, selected, selectedRange }) => {
  const backwardsHandler = () => onDateSelect(moveDate(selected, selectedRange, 'backward'))
  const forwardsHandler = () => onDateSelect(moveDate(selected, selectedRange, 'forward'))

  return (
    <React.Fragment>
      <Button className="sked-calendar-controls__navigate-backwards" data-sk-name="navigate-backwards" buttonType="transparent" onClick={ backwardsHandler }>
        <Icon name="calendarNavLeft" className="sk-h-3" />
      </Button>
      <Button className="sked-calendar-controls__navigate-forwards" data-sk-name="navigate-forwards" buttonType="transparent" onClick={ forwardsHandler }>
...
const DateSelector: React.FC<IDateSelectorProps> = ({ onDateSelect, selectedRange = 'day', selected }) => {  
  const dates = [selected, moveDate(moveDate(selected, selectedRange, 'forward'), 'day', 'backward')]
  const startDate = first(dates)
  const endDate = last(dates)

  const formattedStartDate = startDate.getFullYear() === endDate.getFullYear() ? formatShortDate(startDate) : formatLongDate(startDate)
  const formattedEndDate = formatLongDate(endDate)
  const formattedDate = selectedRange === 'day' ? formatLongDate(selected) : `${formattedStartDate} - ${formattedEndDate}`

  // Need to use the `any` type here as react datepicker will work with our node and then provide the props
  const DateRangeDisplay: React.FC<any> = React.forwardRef(({ onClick }, _ref) => (
    <Button className="sked-calendar-controls__date-range-display" buttonType="transparent" onClick={ onClick } data-sk-name="date-range-display">
      { formattedDate }
...
const RangePicker: React.FC<IRangeProps> = ({ selectedRange = 'day', onRangeChange, rangeOptions = ['day', '3-days', 'week'] }) => {
  const rangeChangeHandler = (option: RangeType) => () => onRangeChange(option)

  return (
    <ButtonDropdown label={ rangeOptionDescription[selectedRange] } data-sk-name="date-range-picker">
      <Menu>
