...
const isSalesforce = credentials && credentials.vendor ? credentials.vendor.type === 'salesforce' : false
...
graphQlRequest$.pipe(
  ...
  try {
    ...
    if (errs) {
      const messages = isArray(errs) ? (errs as {message: string}[]).map((err: {message: string}) => err.message).join(', ') : (errs as Error).message
      toastMessage.error(messages)
    }
  }
})

const makeGraphQlRequest = (query: string, variables?: object) => new Promise<GraphqlResponse>(resolve => {
  graphQlRequest$.next({
    resolve,
    request: {
      query,
...
export const fetchOnePage = async <T>(
  ...
  const {
    data: graphQlData,
    errors
  } = ((await makeGraphQlRequest(query, variables)) as GraphqlResponse<OnePageGraphQlResponse<T>>)
...
const MAX_RECORD_LOAD_PER_BATCH = isSalesforce ? 2000 : Number.POSITIVE_INFINITY

const graphQlOrderByUIDBacthRequest = async <T extends HasUID, K extends keyof T = keyof T>(
  query: string,
  responseKey: string,
  ...
  const filterKey = paginationVariableKey ? paginationVariableKey.filter : 'filter'
  const offsetKey = paginationVariableKey ? paginationVariableKey.offset : 'offset'
  const orderByKey = paginationVariableKey ? paginationVariableKey.orderBy : 'orderBy'
  ...
  const fromItemFilter = orderBy.reduce<{
    ...
    return {
      filter: [...previous.filter, [previous.equalFilter, `${key} > ${lastNodeValue}`].join(' AND ')],
      equalFilter: [previous.equalFilter, `${key} == ${lastNodeValue}`].join(' AND ')
    }
  }, {
    filter: [],
    equalFilter: ''
  })
  const appendFilterValue = [fromItemFilter.filter.map(val => `(${val})`).join(' OR ') || `UID > ${JSON.stringify(fromItem.UID)}`, curentFilterValue].filter(val => !!val).map(val => `(${val})`).join(' AND ')
  ...
  return flatMap(
    ({ data }) => data,
    await Promise.all(
      [...new Array(numberOfPage)]
        .map(
          (item, index) => fetchOnePage<T>(query, responseKey, {
            ...variables,
...
const getNextBatchData = <T extends HasUID>(previousRemainingRecords: number, data: T[]) => ({
  remainingRecords: previousRemainingRecords - data.length,
  fromUID: getLastUID(data)
})

export const autoPaginationGraphqlForSalesforce = async <T extends HasUID, K extends keyof T = keyof T>(
  query: string,
  responseKey: string,
  variables: Record<string, string> = {},
  orderBy: K[],
  ...
  const firstKey = paginationVariableKey ? paginationVariableKey.first : 'first'
  const offsetKey = paginationVariableKey ? paginationVariableKey.offset : 'offset'
  const orderByKey = paginationVariableKey ? paginationVariableKey.orderBy : 'orderBy'

  let resultData: T[] = []
  const { totalCount, data: firstPageDatas } = await fetchOnePage<T>(query, responseKey, {
    ...variables,
  ...
  let {
    remainingRecords,
    fromUID
  } = getNextBatchData(totalCount, firstPageDatas)
...
const autoPaginationGraphqlRequestSkedulo = async <T, K extends keyof T = keyof T>(
  query: string,
  responseKey: string,
  variables: Record<string, string> = {},
  orderBy: K[],
  ...
  const { totalCount, data: firstPageDatas } = await fetchOnePage<T>(query, responseKey, initVariable)
  ...
  const subsequenceData = flatMap(
    ({ data }) => data,
    await Promise.all(
      [...new Array(remainingPage)].map((item, index) => fetchOnePage<T>(
        query, responseKey, {
          ...initVariable,
...
export const autoPaginationGraphqlRequest = async <T extends HasUID, K extends keyof T = keyof T>(
  query: string,
  responseKey: string,
  variables: Record<string, string> = {},
  orderBy: K[],
  ...
  if (isSalesforce) {
    return autoPaginationGraphqlForSalesforce<T, K>(query, responseKey, variables, orderBy, paginationVariableKey)
  }
  return autoPaginationGraphqlRequestSkedulo<T, K>(query, responseKey, variables, orderBy, paginationVariableKey)
}
