...
const toEventRange = (event: CalendarEvent) => {
  const isGoingThroughNoon =
    format(event.start, "a") !== format(event.end, "a");
  const startFormat = `${getHourFormat(event.start)}${
    isGoingThroughNoon ? "a" : ""
  }`;
  const endFormat = `${getHourFormat(event.end)}a`;
  const dateFormat = "d MMM";
  return `${format(event.start, dateFormat)} ${format(
    event.start,
    startFormat
  ).toLowerCase()} - ${format(event.end, dateFormat)} ${format(
    event.end,
    endFormat
  ).toLowerCase()}`;
};
