...
const getDefaultTestProps = (additionalProps = {}, onSort:(data:any) => void = data => {}) => {
  return {
    data: [
...
describe('Table', () => {
  ...
  describe('Sorting', () => {
    it('default', () => {
      ...
      const props = getDefaultTestProps({}, data => { dataFromCallback = data })
    ...
    it('custom with callback', () => {
      ...
      const config = {
        options: {
          sortable: {
            onSort: data => { dataFromCallback = data }
          }
        },
        columns: [
          { key: 'field1' },
          { key: 'field2', sortable: true, sortingFunction: data => {
    ...
    it('displays default sort icon', () => {
      ...
      expect(sortIcons.at(0).hasClass('sked-table-header-cell-icon--sortedBy')).toEqual(true)
      expect(sortIcons.at(1).hasClass('sked-table-header-cell-icon--sortedBy')).toEqual(false)

      wrapper.find(Icon).at(1).simulate('click')
      sortIcons = wrapper.find('[data-sk-name="sort-icon"]')

      expect(wrapper.state('sortDirection')).toEqual('desc')
      expect(sortIcons.at(0).hasClass('sked-table-header-cell-icon--sortedBy')).toEqual(false)
      expect(sortIcons.at(1).hasClass('sked-table-header-cell-icon--sortedBy')).toEqual(true)
    })
  ...
  describe('Select', () => {
    it('prevents to select by keys with non unique values ', () => {
      ...
      expect(() => shallow(<DynamicTable { ...props } config={ { ...props.config, options: { selectable } } } />)).toThrowError()
    })
    ...
    it('setting state and executing callback', () => {
      ...
      const wrapper = mount(<DynamicTable { ...props } config={ { ...props.config, options: { selectable } } } />)

      const selectInput = num => wrapper.find('tr[data-sk-name="sked-table-row-renderer"]').at(num).find(FormInputElement)
      const selectedValue = num => props.data[num].field1

      const toggle = (num, on = true) => selectInput(num).simulate('change', { target: { checked: on } })
      ...
      const selectAll = wrapper.find(TableHeaderRenderer).first().find(FormInputElement)
      ...
      expect(onSelect).toHaveLastReturnedWith(new Set(['value11', 'value21', 'value31']))
    })
