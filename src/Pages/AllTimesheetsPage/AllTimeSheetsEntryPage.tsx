...
interface Props
  extends Pick<RootState, "filter" | "timeSheetEntries" | "user" | "order"> {
...
class AllTimeSheetsEntryPage extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    // }
    const { dateRange: prevDateRange, filterValues: prevFilterValues } =
      prevProps.filter;
    ...
    const isDiffFilters =
      JSON.stringify(
        filterValues.map((value) => value.filterValues).filter(Boolean)
      ) !==
      JSON.stringify(
        prevFilterValues.map((value) => value.filterValues).filter(Boolean)
      );
  ...
  preparePaginationParams = () => {
    ...
    return {
      ...
      page: currentPage * ITEMS_PER_PAGE,
    };
  };
  ...
  getNewEntries = (
    ...
    const newTimeSheetEntries = this.props.timeSheetEntries.values.filter(
      (entry) => {
  ...
  onSubmit = (timeSheetEntryId: string | string[], comment: string) => {
    ...
    if (isArray(timeSheetEntryId)) {
      timeSheetId = timeSheetEntryId.map((id) =>
        selectTimeSheetIdByEntryId(this.props.timeSheetEntries.values, id)
      );
    } else {
      timeSheetId = selectTimeSheetIdByEntryId(
        this.props.timeSheetEntries.values,
        timeSheetEntryId
      );
    }
  ...
  onApprove = async (
    ...
    if (!isArray(timeSheetEntryId)) {
      ...
      newEntryIds = this.getNewEntries(timeSheetEntryId).map(
        (entry) => entry.UID
      );
      timeSheetIds = [
        selectTimeSheetIdByEntryId(
      ...
      for (const entryId of timeSheetEntryId) {
        jobAllocationIds = jobAllocationIds.concat(
          this.getJobAllocationIds(entryId)
        );
        newEntryIds = newEntryIds.concat(
          this.getNewEntries(entryId).map((entry) => entry.UID)
        );
        timeSheetIds = timeSheetIds.concat(
          selectTimeSheetIdByEntryId(
            this.props.timeSheetEntries.values,
            entryId
          )
  ...
  onRecall = (timeSheetEntryId: string, timeSheetStatus: string) => {
    ...
    const timeSheetId = selectTimeSheetIdByEntryId(
      this.props.timeSheetEntries.values,
      timeSheetEntryId
    );
    if (timeSheetStatus === TIME_SHEET_STATUS.APPROVED)
      newStatus = TIME_SHEET_STATUS.SUBMITTED;
    if (timeSheetStatus === TIME_SHEET_STATUS.REJECTED)
      newStatus = TIME_SHEET_STATUS.SUBMITTED;
    if (timeSheetStatus === TIME_SHEET_STATUS.SUBMITTED)
      newStatus = TIME_SHEET_STATUS.DRAFT;
