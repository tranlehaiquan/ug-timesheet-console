...
type FormElementProps = InputFormValidationProps &
  React.InputHTMLAttributes<HTMLInputElement>;

const FormField: React.FunctionComponent<
  ValidationProps & React.HTMLAttributes<HTMLDivElement>
> = ({ validation, className, children, ...otherProps }) => {
  return (
    <div
      className={classnames(className, "sked-form-element", {
        "sked-form-element--invalid": validation && !validation.isValid,
      })}
      {...otherProps}
    >
      {children}
...
const ShowErrorIfInvalid = (validation: ValidationProps["validation"]) => {
  if (validation && !validation.isValid && validation.error) {
    return (
      <div className="sked-form-element__errors" data-sk-name="form-error-text">
        {validation.error}
      </div>
    );
  }
 ...
 */
export class FormInputElement extends React.PureComponent<
  FormElementProps,
  {}
> {
      ...
      <div className="sk-flex sk-items-center">
        ...
        <FormLabel
          ...
          className={classnames(
            { "sk-cursor-default": disabled, "sk-cursor-pointer": !disabled },
            inlineLabelClassName
          )}
        >
          {inlineLabel}
  ...
  render() {
    ...
    const inputClasses = classnames(
      { "sked-input-textbox sked-form-element__outline": this.isOfTextType() },
      this._inputClassNames(type),
      className
    );
    ...
    return inlineLabel
      ? this.withInlineLabel(inputComponent, this.props)
      : inputComponent;
  }
}
  ...
  return (
    <div className={className} {...otherProps}>
      <div className={inputSizes[size || "full"]}>
        {isReadOnly ? (
          <ReadOnly
            placeholderValue={readOnlyPlaceholder}
            value={readOnlyValue}
            name={name}
            locked={locked}
          />
        ) : (
          <FormField validation={validation}>{children}</FormField>
        )}
      </div>
 ...
 */
export const FormValidationCallout: React.FunctionComponent<
  ValidationProps & React.HTMLAttributes<HTMLDivElement>
> = ({ validation, className, ...otherProps }) => {
