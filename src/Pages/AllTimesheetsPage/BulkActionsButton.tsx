...
} from "../../components/timesheets-table/UpdateStatusModal";
...
const getSelectedTimeSheetEntries = (
  entries: ReduxDataTypes.TimesheetEntry[],
  selectedTimeSheetEntryUIDs: Set<string>
) => {
  return [...selectedTimeSheetEntryUIDs].map((uid) =>
    entries.find((entry) => entry.UID === uid)
  );
};

export const BulkActionsButton: React.FC<Props> = ({
  selectedTimeSheetEntryUIDs,
  onReject,
  onApprove,
}) => {
  const selectedTimeSheetEntries = useSelector((state: RootState) =>
  ...
  const [updateModalProps, setUpdateModalProps] = React.useState<
    Partial<UpdateStatusModalProps>
  >({ isOpened: false });

  const onRejectClick = () => {
    const rejectableTimeSheetEntries =
      getTimeSheetEntriesWithoutStatus("Rejected");
    if (
      selectedTimeSheetEntryUIDs.size === 1 &&
      rejectableTimeSheetEntries.length === 1
    ) {
      openSingleUpdateModal(
        rejectableTimeSheetEntries[0].UID,
        "reject",
        onReject
      );
    } else {
      openBatchUpdateModal(
        rejectableTimeSheetEntries.map((entry) => entry.UID),
        onReject,
        "Reject timesheets",
        "rejected"
      );
  ...
  const onApproveClick = () => {
    const approvedTimeSheetEntries =
      getTimeSheetEntriesWithoutStatus("Approved");
    if (
      selectedTimeSheetEntryUIDs.size === 1 &&
      approvedTimeSheetEntries.length === 1
    ) {
      openSingleUpdateModal(
        approvedTimeSheetEntries[0].UID,
        "approve",
        onApprove
      );
    } else {
      openBatchUpdateModal(
        approvedTimeSheetEntries.map((entry) => entry.UID),
        onApprove,
        "Approve timesheets",
        "approved"
      );
  ...
  const getTimeSheetEntriesWithoutStatus = (status: string) => {
    return selectedTimeSheetEntries.filter(
      (entry) => entry.Timesheet.Status !== status
    );
  };
